...
def fileview(request, id):
    ...
    ids = (
        File.objects.filter(uploader=request.user.extrainfo)
    ...
    for i in ids:
        file_tracking_ids = Tracking.objects.filter(file_id=i).values_list(
            "id", flat=True
        )
...
def fileview1(request, id):
    ...
    outward_files = (
        ...
        .filter(current_id=request.user.extrainfo)
...
def fileview2(request, id):
    ...
    inward_file = (
        ...
        .filter(receiver_id=request.user)
...
def archive_finish(request, id):
    ...
    return render(
        request, "filetracking/archive_finish.html", {"file": file1, "track": track}
    )
...
def finish_fileview(request, id):
    ...
    out = (
        ...
        .filter(file_id__uploader=request.user.extrainfo, is_read=False)
        .order_by("-forward_date")
    )
