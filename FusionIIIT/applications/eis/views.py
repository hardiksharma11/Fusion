...
from applications.office_module.models import (
    Project_Closure,
    Project_Extension,
...
def profile(request, username=None):
    ...
    project_ext = Project_Extension.objects.filter(project_id__PI_id=pf).order_by(
        "project_id__PI_id__user"
    )
    project_closure = Project_Closure.objects.filter(project_id__PI_id=pf).order_by(
        "project_id__PI_id__user"
    )
    project_reall = Project_Reallocation.objects.filter(project_id__PI_id=pf).order_by(
        "project_id__PI_id__user"
    )
    ############################

    journal = emp_research_papers.objects.filter(pf_no=pf, rtype="Journal").order_by(
        "-year"
    )
    conference = emp_research_papers.objects.filter(
        pf_no=pf, rtype="Conference"
    ).order_by("-year")
    ...
    consultancy = emp_consultancy_projects.objects.filter(pf_no=pf).order_by(
        "-date_entry"
    )
    ...
    mtechs = emp_mtechphd_thesis.objects.filter(pf_no=pf, degree_type=1).order_by(
        "-date_entry"
    )
    phds = emp_mtechphd_thesis.objects.filter(pf_no=pf, degree_type=2).order_by(
        "-date_entry"
    )
    ...
    try:
        ...
        pers = None
    # edited 26March
    a1 = HoldsDesignation.objects.select_related(
...
def rspc_profile(request):
    ...
    journal = emp_research_papers.objects.filter(rtype="Journal").order_by(
        "-year", "-a_month"
    )
    conference = emp_research_papers.objects.filter(rtype="Conference").order_by(
        "-year", "-a_month"
    )
    ...
    mtechs = emp_mtechphd_thesis.objects.filter(degree_type=1).order_by(
        "-s_year", "-a_month"
    )
    phds = emp_mtechphd_thesis.objects.filter(degree_type=2).order_by(
        "-s_year", "-a_month"
    )
...
def fvisit_insert(request):
    ...
    try:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("start_date"), "%B %d, %Y"
        )
    except:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("start_date"), "%b. %d, %Y"
    ...
    try:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("end_date"), "%B %d, %Y"
        )
    except:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("end_date"), "%b. %d, %Y"
...
def ivisit_insert(request):
    ...
    try:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("start_date2"), "%B %d, %Y"
        )
    except:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("start_date2"), "%b. %d, %Y"
    ...
    try:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("end_date2"), "%B %d, %Y"
        )
    except:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("end_date2"), "%b. %d, %Y"
...
def journal_insert(request):
    ...
    if year != "":
        eis.year = year
    if (
        request.POST.get("doi") != None
        and request.POST.get("doi") != ""
        and request.POST.get("doi") != "None"
    ):
        try:
    ...
    if (
        request.POST.get("doa") != None
        and request.POST.get("doa") != ""
        and request.POST.get("doa") != "None"
    ):
        try:
    ...
    if (
        request.POST.get("dop") != None
        and request.POST.get("dop") != ""
        and request.POST.get("dop") != "None"
    ):
        try:
    ...
    if (
        request.POST.get("dos") != None
        and request.POST.get("dos") != ""
        and request.POST.get("dos") != "None"
    ):
        try:
...
def conference_insert(request):
    ...
    if (
        request.POST.get("doi3") != None
        and request.POST.get("doi3") != ""
        and request.POST.get("doi3") != "None"
    ):
        ...
        try:
            ...
            try:
                ...
                eis.doi = x
    if (
        request.POST.get("doa3") != None
        and request.POST.get("doa3") != ""
        and request.POST.get("doa3") != "None"
    ):
    ...
    if (
        request.POST.get("dop3") != None
        and request.POST.get("dop3") != ""
        and request.POST.get("dop3") != "None"
    ):
        ...
        try:
            ...
            eis.date_publication = datetime.datetime.strptime(x, "%b. %d, %Y")
    if (
        request.POST.get("dos3") != None
        and request.POST.get("dos3") != ""
        and request.POST.get("dos3") != "None"
    ):
...
def event_insert(request):
    ...
    try:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("event_start_date"), "%B %d, %Y"
        )
    except:
        eis.start_date = datetime.datetime.strptime(
            request.POST.get("event_start_date"), "%b. %d, %Y"
    ...
    try:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("event_end_date"), "%B %d, %Y"
        )
    except:
        eis.end_date = datetime.datetime.strptime(
            request.POST.get("event_end_date"), "%b. %d, %Y"
...
def project_insert(request):
    ...
    if request.POST.get("project_id") == None or request.POST.get("project_id") == "":
        ...
        eis = get_object_or_404(
            emp_research_projects, id=request.POST.get("project_id")
        )
    eis.user = request.user
    eis.pf_no = pf
    ...
    if (
        ...
        and request.POST.get("start") != "None"
    ):
        try:
            eis.start_date = datetime.datetime.strptime(x, "%B %d, %Y")
        except:
    ...
    if x[:5] == "Sept.":
        x = "Sep." + x[5:]
    if (
        request.POST.get("sub") != None
        and request.POST.get("sub") != ""
        and request.POST.get("sub") != "None"
    ):
        try:
            eis.date_submission = datetime.datetime.strptime(x, "%B %d, %Y")
        except:
...
def consult_insert(request):
    ...
    if (
        request.POST.get("consultancy_id") == None
        or request.POST.get("consultancy_id") == ""
    ):
        eis = emp_consultancy_projects()
    else:
        eis = get_object_or_404(
            emp_consultancy_projects, id=request.POST.get("consultancy_id")
        )
    eis.user = request.user
    eis.pf_no = pf
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for row in reader:
                ...
                try:
                    if (
                        row["l_date"] != "0000-00-00 00:00:00"
                        and row["l_date"] != "0000-00-00"
                    ):
                        e.l_date = row["l_date"]
        ...
        if form.is_valid():
            ...
            for row in reader:
                ...
                if (
                    ...
                    or row["status"] == "Communicated"
                ):
                    e.status = row["status"]

                try:
                    if (
                        str(row["date_submission"]) == " "
                        or str(row["date_submission"]) == ""
                        or row["date_submission"] == None
                    ):
                        a = 1
                    else:
                        if (
                            row["date_submission"] != "0000-00-00 00:00:00"
                            and row["date_submission"] != "0000-00-00"
                        ):
                            e.date_submission = row["date_submission"]
                            e.date_submission = datetime.datetime.strptime(
                                e.date_submission, "%Y-%m-%d"
                            ).date()
...
def generate_report(request):
    ...
    if request.POST.get("journal_select") == "journal":
        journal = (
            emp_research_papers.objects.filter(pf_no=pf, rtype="Journal")
            .filter(year__range=[start, end])
            .order_by("-date_entry")
        )
        journal_req = "1"
    else:
    ...
    if request.POST.get("conference_select") == "conference":
        conference = (
            emp_research_papers.objects.filter(pf_no=pf, rtype="Conference")
            .filter(year__range=[start, end])
            .order_by("-date_entry")
        )
        conference_req = "1"
    ...
    if request.POST.get("projects_select") == "projects":
        projects = (
            emp_research_projects.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-start_date")
        )
        projects_req = "1"
    else:
    ...
    if request.POST.get("consultancy_select") == "consultancy":
        consultancy = (
            emp_consultancy_projects.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("patents_select") == "patents":
        patents = (
            emp_patents.objects.filter(pf_no=pf)
            .filter(p_year__range=[start, end])
            .filter(a_month__range=[star, en])
            .order_by("-date_entry")
        )
        patents_req = "1"
    else:
    ...
    if request.POST.get("techtransfers_select") == "techtransfers":
        techtransfers = (
            emp_techtransfer.objects.filter(pf_no=pf)
            .filter(date_entry__year__range=[start, end])
            .filter(date_entry__month__range=[star, en])
            .order_by("-date_entry")
        )
        techtransfers_req = "1"
    else:
    ...
    if request.POST.get("mtechs_select") == "mtechs":
        mtechs = (
            emp_mtechphd_thesis.objects.filter(pf_no=pf, degree_type=1)
            .filter(s_year__range=[start, end])
            .filter(a_month__range=[star, en])
            .order_by("-date_entry")
        )
        mtechs_req = "1"
    ...
    if request.POST.get("phds_select") == "phds":
        phds = (
            emp_mtechphd_thesis.objects.filter(pf_no=pf, degree_type=2)
            .filter(s_year__range=[start, end])
            .filter(a_month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("fvisits_select") == "fvisits":
        fvisits = (
            emp_visits.objects.filter(pf_no=pf, v_type=2)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-entry_date")
        )
    ...
    if request.POST.get("ivisits_select") == "ivisits":
        ivisits = (
            emp_visits.objects.filter(pf_no=pf, v_type=1)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-entry_date")
        )
    ...
    if request.POST.get("consymps_select") == "consymps":
        consymps = (
            emp_confrence_organised.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("awards_select") == "awards":
        awards = (
            emp_achievement.objects.filter(pf_no=pf)
            .filter(a_year__range=[start, end])
            .order_by("-date_entry")
        )
        awards_req = "1"
    else:
    ...
    if request.POST.get("talks_select") == "talks":
        talks = (
            emp_expert_lectures.objects.filter(pf_no=pf)
            .filter(l_date__year__range=[start, end])
            .filter(l_date__month__range=[star, en])
            .order_by("-date_entry")
        )
        talks_req = "1"
    else:
    ...
    if request.POST.get("chairs_select") == "chairs":
        chairs = (
            emp_session_chair.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("keynotes_select") == "keynotes":
        keynotes = (
            emp_keynote_address.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("events_select") == "events":
        events = (
            emp_event_organized.objects.filter(pf_no=pf)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-start_date")
        )
...
def rspc_generate_report(request):
    ...
    if request.POST.get("journal_select") == "journal":
        journal = (
            emp_research_papers.objects.filter(rtype="Journal")
            .filter(year__range=[start, end])
            .order_by("-date_entry")
        )
        journal_req = "1"
    ...
    if request.POST.get("conference_select") == "conference":
        conference = (
            emp_research_papers.objects.filter(rtype="Conference")
            .filter(year__range=[start, end])
            .order_by("-date_entry")
        )
        conference_req = "1"
    ...
    if request.POST.get("projects_select") == "projects":
        projects = (
            emp_research_projects.objects.all()
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-start_date")
        )
    ...
    if request.POST.get("consultancy_select") == "consultancy":
        consultancy = (
            emp_consultancy_projects.objects.all()
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("mtechs_select") == "mtechs":
        mtechs = (
            emp_mtechphd_thesis.objects.filter(degree_type=1)
            .filter(s_year__range=[start, end])
            .filter(a_month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("phds_select") == "phds":
        phds = (
            emp_mtechphd_thesis.objects.filter(degree_type=2)
            .filter(s_year__range=[start, end])
            .filter(a_month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("fvisits_select") == "fvisits":
        fvisits = (
            emp_visits.objects.filter(v_type=2)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-entry_date")
        )
    ...
    if request.POST.get("ivisits_select") == "ivisits":
        ivisits = (
            emp_visits.objects.filter(v_type=1)
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-entry_date")
        )
    ...
    if request.POST.get("consymps_select") == "consymps":
        consymps = (
            emp_confrence_organised.objects.all()
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("talks_select") == "talks":
        talks = (
            emp_expert_lectures.objects.all()
            .filter(l_date__year__range=[start, end])
            .filter(l_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("keynotes_select") == "keynotes":
        keynotes = (
            emp_keynote_address.objects.all()
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-date_entry")
        )
    ...
    if request.POST.get("events_select") == "events":
        events = (
            emp_event_organized.objects.all()
            .filter(start_date__year__range=[start, end])
            .filter(start_date__month__range=[star, en])
            .order_by("-start_date")
        )
