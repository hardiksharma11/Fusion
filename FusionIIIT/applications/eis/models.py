...
class emp_visits(models.Model):
    ...
    def __str__(self):
        return "PF No.: {}   Name: {}   Purpose: {}".format(
            self.pf_no, self.country, self.purpose
        )
...
class emp_research_papers(models.Model):
    ...
    rtype = models.CharField(
        max_length=500, choices=R_TYPE_CHOICES, default="Conference"
    )
    ...
    is_sci = models.CharField(
        max_length=6, choices=IS_SCI_TYPE_CHOICES, null=True, blank=True
    )
    ...
    for r in range(1, 13):
        MONTH_CHOICES.append((r, r))
    a_month = models.CharField(
        max_length=500, choices=MONTH_CHOICES, null=True, blank=True, default=1
    )
    ...
    def __str__(self):
        return "PF No.: {}   Author: {}  Title: {}".format(
            self.pf_no, self.authors, self.title_paper
        )
...
class emp_published_books(models.Model):
    ...
    def __str__(self):
        return "PF No.: {}   Type: {}  Title: {}".format(
            self.pf_no, self.p_type, self.title
        )
...
class emp_patents(models.Model):
    ...
    def __str__(self):
        return "PF No.: {}   Status: {}  Title: {}".format(
            self.pf_no, self.status, self.title
        )
...
class emp_mtechphd_thesis(models.Model):
    ...
    status = models.CharField(
        max_length=10, choices=STATUS_TYPE_CHOICES, null=True, blank=True
    )

    def __str__(self):
        return "PF No.: {}   Supervisor: {}  Title: {}".format(
            self.pf_no, self.supervisors, self.title
        )
...
class emp_keynote_address(models.Model):
    ...
    type = models.CharField(
        max_length=140, choices=KEYNOTE_TYPE_CHOICES, default="Keynote"
    )
    ...
    def __str__(self):
        return "PF No.: {}   Name: {}  Title: {}".format(
            self.pf_no, self.name, self.title
        )
