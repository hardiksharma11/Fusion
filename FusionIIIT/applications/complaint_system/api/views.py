...
def complaint_details_api(request, detailcomp_id1):
    ...
    complaint_detail_serialized = serializers.StudentComplainSerializers(
    ...
    if complaint_detail.worker_id is None:
        ...
        worker_detail_serialized = serializers.WorkersSerializers(
            instance=worker_detail
        ).data
    ...
    complainer_extra_info_serialized = serializers.ExtraInfoSerializers(
        instance=complainer_extra_info
    ).data
    response = {
...
def edit_complain_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Complain does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
    if request.method == "DELETE":
        complain.delete()
        return Response(
            {"message": "Complain deleted"}, status=status.HTTP_404_NOT_FOUND
        )
...
def edit_worker_api(request, w_id):
    ...
    try:
        ...
        return Response(
            {"message": "The worker does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
    if request.method == "DELETE":
        worker.delete()
...
def edit_caretaker_api(request, c_id):
    ...
    try:
        ...
        return Response(
            {"message": "The Caretaker does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        caretaker.delete()
        return Response(
            {"message": "Caretaker deleted"}, status=status.HTTP_404_NOT_FOUND
        )
    elif request.method == "PUT":
