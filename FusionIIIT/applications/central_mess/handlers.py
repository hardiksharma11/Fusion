...
def add_vacation_food_request(request, student):
    ...
    vacation_object = Vacation_food(
        student_id=student, start_date=start_date, end_date=end_date, purpose=purpose
    )
...
def add_menu_change_request(request, student):
    ...
    try:
        ...
        menu_object = Menu_change_request(
            dish=dish, student_id=student, request=new_dish, reason=reason
        )
        menu_object.save()
        data = {"status": 1}
        return data
...
def add_mess_registration_time(request):
    ...
    if start_reg > end_reg or start_reg < date_today:
        ...
        mess_reg_obj.save()
        data = {"status": 1, "message": "Registration Started Successfully"}
        return data
...
def handle_rebate_response(request):
    ...
    if action == "2":
        ...
        message = (
            "Your leave request has been rejected between dates "
            + str(b.date())
            + " and "
            + str(d.date())
        )
...
def add_mess_committee(request, roll_number):
    ...
    check_obj = HoldsDesignation.objects.filter(
        Q(user__username=roll_number)
        & (
            Q(designation__name__contains="mess_committee")
            | Q(designation__name__contains="mess_convener")
        )
    ...
    if check_obj:
        ...
        designation_object = HoldsDesignation(
            user=add_user, working=add_user, designation=designation
        )
        designation_object.save()
        central_mess_notif(request.user, add_user, "added_committee", "")
        data = {"status": 1, "message": roll_number + " is added to Mess Committee"}
        return data
...
def generate_bill():
    ...
    for student in student_all:
            ...
            if r.status == "2":
                if r.start_date.month == month_g:
                    if r.end_date.month == today_g:
                        rebate_count = (
                            rebate_count
                            + abs((last_day_prev_month - r.start_date).days)
                            + 1
                        )
                    else:
                        rebate_count = (
                    ...
                    rebate_count = (
                        rebate_count + abs((r.end_date - first_day_prev_month).days) + 1
                    )
                else:
                    rebate_count = 0
        ...
        if Monthly_bill.objects.filter(
            student_id=student,
            month=previous_month,
            year=previous_month_year,
            total_bill=total,
            ...
            print("exists")
        elif Monthly_bill.objects.filter(
            student_id=student, month=previous_month, year=previous_month_year
        ):
            Monthly_bill.objects.filter(
                student_id=student, month=previous_month, year=previous_month_year
            ).update(
                student_id=student,
                month=previous_month,
