...
from .forms import (
    AcademicReplacementForm,
    AdminReplacementForm,
    BaseLeaveFormSet,
    EmployeeCommonForm,
    LeaveSegmentForm,
    StudentApplicationForm,
    AcademicReplacementFormOffline,
    AdminReplacementFormOffline,
    BaseLeaveFormSetOffline,
    EmployeeCommonFormOffline,
...
LeaveFormSet = formset_factory(
    LeaveSegmentForm, extra=0, max_num=3, min_num=1, formset=BaseLeaveFormSet
)
...
LeaveFormSetOffline = formset_factory(
    LeaveSegmentFormOffline,
...
AcadFormSetOffline = formset_factory(
    AcademicReplacementFormOffline, extra=0, max_num=3, min_num=1
)
AdminFormSetOffline = formset_factory(
    AdminReplacementFormOffline, extra=0, max_num=3, min_num=1
)
...
def handle_faculty_leave_application(request):
    leave_form_set = LeaveFormSet(
        request.POST, request.FILES, prefix="leave_form", user=request.user
    )
    academic_form_set = AcadFormSet(
        request.POST, prefix="acad_form", form_kwargs={"user": request.user}
    )
    admin_form_set = AdminFormSet(
        request.POST, prefix="admin_form", form_kwargs={"user": request.user}
    )
    ...
    if leave_valid and acad_valid and admin_valid and common_valid:
        leave = Leave(applicant=request.user)
        segments = list()
    ...
    user_leave_applications = Leave.objects.filter(applicant=request.user).order_by(
        "-timestamp"
    )
    processed_requests = request.user.all_leave_requests.filter(~Q(status="pending"))
    user_leave_applications_offline = LeaveOffline.objects.filter(
...
def handle_student_leave_application(request):
    ...
    if form.is_valid():
        ...
        LeaveRequest.objects.create(leave=leave, requested_from=requested_from)
        deduct_leave_balance(leave, False)
        messages.add_message(request, messages.SUCCESS, "Successfully Submitted !")
...
def send_faculty_leave_form(request):
    ...
    context = {
        ...
        "acad_form_set": AcadFormSet(
            prefix="acad_form", form_kwargs={"user": request.user}
        ),
        "admin_form_set": AdminFormSet(
            prefix="admin_form", form_kwargs={"user": request.user}
        ),
        "common_form": common_form,
        "forms": True,
        "rep_segments": rep_segments,
...
def intermediary_processing(request, leave_request):
    ...
    if status == "forward":
        ...
        leave_request.save()
        leave_module_notif(
            request.user, leave_request.leave.applicant, "leave_forwarded"
        )
        ...
        leave.save()
        leave_module_notif(
            request.user, leave_request.leave.applicant, "leave_rejected"
        )
...
def authority_processing(request, leave_request):
    ...
    if status == "accept":
        ...
        leave_module_notif(
            request.user, leave_request.leave.applicant, "leave_accepted"
        )
        ...
        LeaveRequest.objects.create(
            leave=leave, requested_from=officer, permission="sanc_off"
        )
        message = "Successfully Forwarded"
    ...
    if is_replacement_request:
        ...
        with transaction.atomic():
            ...
            if status == "accept":
                ...
                rep_request.save()
                leave_module_notif(
                    request.user, rep_request.leave.applicant, "request_accepted"
                )
                if rep_request.leave.relacements_accepted():
                    ...
                    authority = (
                        rep_request.leave.applicant.leave_admins.authority.designees.first().user
                    )
                    leave_module_notif(
                        rep_request.leave.applicant, authority, "leave_request"
                    )
                    LeaveRequest.objects.create(
                        leave=rep_request.leave,
                        requested_from=authority,
                ...
                leave.save()
                leave_module_notif(
                    request.user, rep_request.leave.applicant, "request_declined"
                )
                leave.replace_segments.filter(status="pending").update(
...
def delete_leave_application(request):
    ...
    if (
        leave
        and leave.applicant == request.user
        and leave.status != "rejected"
        and timezone.now().date() <= leave_start_date
    ):
        ...
        for i in rep_requests:
            if i.status == "accepted":
                ...
                leave_module_notif(
                    request.user, i.replacer, "replacement_cancel", str(i.start_date)
                )
        ...
        response = JsonResponse(
            {"status": "success", "message": "Successfully Cancelled"}
        )
    else:
        response = JsonResponse(
            {"status": "failed", "message": "Cancellation Failed"}, status=400
        )
...
def handle_offline_leave_application(request):
    ...
    try:
        leave_form_set = LeaveFormSetOffline(
            request.POST, request.FILES, prefix="leave_form_offline"
        )
    ...
    if leave_valid and admin_valid and common_valid and type_of_user == "faculty":
        ...
        if academic_form_set.is_valid():
            leave = LeaveOffline(applicant=leave_user)
    ...
    user_leave_applications = LeaveOffline.objects.filter(
