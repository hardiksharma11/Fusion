...
class LeavesCount(models.Model):
    ...
    def __str__(self):
        return "{} in {} has {} {}s".format(
            self.user.username, self.year, self.remaining_leaves, self.leave_type
        )
...
class Leave(models.Model):
    applicant = models.ForeignKey(
        User, related_name="all_leaves", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return "{} applied, status: {}".format(self.applicant.username, self.status)
...
class ReplacementSegment(models.Model):
    ...
    replacement_type = models.CharField(
        max_length=20, default="academic", choices=Constants.REPLACEMENT_TYPES
    )
...
class LeaveRequest(models.Model):
    ...
    requested_from = models.ForeignKey(
        User, related_name="all_leave_requests", on_delete=models.CASCADE
    )
    remark = models.CharField(max_length=50, blank=True, null=True)
    permission = models.CharField(
        max_length=20, default="sanc_auth", choices=Constants.LEAVE_PERMISSIONS
    )
    status = models.CharField(
        max_length=20, default="pending", choices=Constants.STATUS
    )
    ...
    def __str__(self):
        return "{} requested {}, {}".format(
            self.leave.applicant.username, self.requested_from.username, self.permission
        )
...
class LeaveAdministrators(models.Model):
    ...
    authority = models.ForeignKey(
        Designation,
        null=True,
        related_name="sanc_authority_of",
        on_delete=models.SET_NULL,
    )
    officer = models.ForeignKey(
        Designation,
        null=True,
        related_name="sanc_officer_of",
        on_delete=models.SET_NULL,
    )
...
class LeaveMigration(models.Model):
    ...
    type_migration = models.CharField(
        max_length=10, default="transfer", choices=Constants.MIGRATION_CHOICES
    )
    on_date = models.DateField(null=False)
    replacee = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="replacee_migrations"
    )
    replacer = models.ForeignKey(
        User, related_name="replacer_migrations", on_delete=models.CASCADE
    )
    replacement_type = models.CharField(
        max_length=20, default="academic", choices=Constants.REPLACEMENT_TYPES
    )
    ...
    def __str__(self):
        return "{} : {}, type => {}".format(
            self.replacee.username, self.replacer.username, self.type_migration
        )
...
class LeaveOffline(models.Model):
    ...
    def __str__(self):
        return "{} applied, status: {}".format(self.applicant.username, "succeed")
