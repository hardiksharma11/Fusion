...
from .helpers import (
    ...
    get_user_choices,
    get_vacation_leave_count,
)


class StudentApplicationForm(forms.Form):

    STUDENT_LEAVE_CHOICES = (("Casual", "Casual"), ("Medical", "Medical"))
...
"""
        ...
        count = get_leave_days(
            data.get("start_date"), data.get("end_date"), leave_type, False, False
        )

        remaining_leaves = LeavesCount.objects.get(
            user=self.user, leave_type=leave_type
        ).remaining_leaves
        if remaining_leaves < count:
            errors["leave_type"] = (
                f"You have only {remaining_leaves} {leave_type.name} leaves"
                " remaining."
            )
    ...
    def clean(self, *args, **kwargs):
        ...
        def check_special_leave_overlap(start_date, end_date, leave_type_id):
            ...
            if leave_type.name.lower() in ["restricted"]:
                ...
                if count < 0:
                    return (
                        "The period for this leave doesn't match with restricted holiday calendar"
                        ". Check Academic Calendar."
                    )
            elif leave_type.name.lower() in ["vacation"]:
                count = get_vacation_leave_count(
                    start_date, end_date, leave_type.name.lower()
                )
                if count < 0:
                    return (
                        "The period for this leave doesn't match with vacation holidays"
                        ". Check Academic Calendar."
                    )
        ...
        if data["start_date"] < data["end_date"]:
            error = check_special_leave_overlap(
                data.get("start_date"), data.get("end_date"), data.get("leave_type")
            )
...
class AdminReplacementForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # print(USER_CHOICES)
        self.fields["admin_rep"] = forms.ChoiceField(
            label="Administrative Responsibility To: ", choices=USER_CHOICES1
        )
    ...
    def clean(self):
        ...
        if start_date > end_date:
            errors["admin_start_date"] = [
                "Administrative Start Date must not be more than the End Date"
            ]
...
class AcademicReplacementForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["acad_rep"] = forms.ChoiceField(
            label="Academic Responsibility To: ", choices=USER_CHOICES1
        )
...
class EmployeeCommonFormOffline(forms.Form):
    ...
    leave_user_select = forms.ModelChoiceField(
        queryset=User.objects.filter(extrainfo__user_type__in=["faculty", "staff"])
    )
...
class AdminReplacementFormOffline(forms.Form):
    ...
    admin_end_date = forms.DateField(label="To")
    # print(USER_CHOICES)
    admin_rep = forms.ModelChoiceField(
        queryset=User.objects.filter(extrainfo__user_type__in=["faculty", "staff"])
    )
    ...
    def clean(self):
        ...
        if start_date > end_date:
            errors["admin_start_date"] = [
                "Administrative Start Date must not be more than End Date"
            ]
...
class AcademicReplacementFormOffline(forms.Form):
    acad_start_date = forms.DateField(label="From")
    acad_end_date = forms.DateField(label="To")
    acad_rep = forms.ModelChoiceField(
        queryset=User.objects.filter(extrainfo__user_type__in=["faculty", "staff"])
    )
...
class BaseLeaveFormSetOffline(BaseFormSet):
    ...
    def clean(self):
        ...
        leave_counts = LeavesCount.objects.filter(
            user=User.objects.get(username=data["leave_user_select"]), year=curr_year
        )
        mapping = dict()
        for form in self.forms:
            try:
