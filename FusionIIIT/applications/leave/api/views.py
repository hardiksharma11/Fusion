...
def replacement_segment_api(request):
    ...
    if request.method == "GET":
        ...
        replacement_segment = serializers.ReplacementSegmentSerializers(
            replacement_segment, many=True
        ).data
        resp = {
...
def replacement_segment_offline_api(request):
    ...
    if request.method == "GET":
        ...
        replacement_segment_offline = serializers.ReplacementSegmentOfflineSerializers(
            replacement_segment_offline, many=True
        ).data
        resp = {
...
def leave_segment_offline_api(request):
    ...
    if request.method == "GET":
        ...
        leave_segment_offline = serializers.LeaveSegmentOfflineSerializers(
            leave_segment_offline, many=True
        ).data
        resp = {
...
def leave_segment_api(request):
    ...
    if request.method == "GET":
        ...
        leave_segment = serializers.LeaveSegmentSerializers(
            leave_segment, many=True
        ).data
        resp = {
...
def leave_request_api(request):
    ...
    if request.method == "GET":
        ...
        leave_request = serializers.LeaveRequestSerializers(
            leave_request, many=True
        ).data
        resp = {
...
def leave_administrators_api(request):
    ...
    if request.method == "GET":
        ...
        leave_administrators = serializers.LeaveAdministratorsSerializers(
            leave_administrators, many=True
        ).data
        resp = {
...
def leave_migration_api(request):
    ...
    if request.method == "GET":
        ...
        leave_migration = serializers.LeaveMigrationSerializers(
            leave_migration, many=True
        ).data
        resp = {
...
def restricted_holiday_api(request):
    ...
    if request.method == "GET":
        ...
        restricted_holiday = serializers.RestrictedHolidaySerializers(
            restricted_holiday, many=True
        ).data
        resp = {
...
def closed_holiday_api(request):
    ...
    if request.method == "GET":
        ...
        closed_holiday = serializers.ClosedHolidaySerializers(
            closed_holiday, many=True
        ).data
        resp = {
...
def vacation_holiday_api(request):
    ...
    if request.method == "GET":
        ...
        vacation_holiday = serializers.VacationHolidaySerializers(
            vacation_holiday, many=True
        ).data
        resp = {
...
def leave_offline_api(request):
    ...
    if request.method == "GET":
        ...
        leave_offline = serializers.LeaveOfflineSerializers(
            leave_offline, many=True
        ).data
        resp = {
...
def edit_leave_type_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Type does not exist"},
...
def edit_leave_count_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Count does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
...
def edit_leave_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave does not exist"}, status=status.HTTP_404_NOT_FOUND
        )
    if request.method == "DELETE":
        obj.delete()
...
def edit_replacement_segment_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Replacement Segment does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Replacement Segment deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
        serializer = serializers.ReplacementSegmentSerializers(
            complain, data=request.data
        )
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
...
def edit_replacement_segment_offline_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Replacement Segment Offline does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Replacement Segment Offline deleted"},
            status=status.HTTP_200_OK,
        )
    elif request.method == "PUT":
        serializer = serializers.ReplacementSegmentOfflineSerializers(
            complain, data=request.data
        )
        if serializer.is_valid():
            serializer.save()
...
def edit_leave_segment_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Segment does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
...
def edit_leave_segment_offline_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Segment Offline does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Leave Segment Offline deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
        serializer = serializers.LeaveSegmentOfflineSerializers(
            complain, data=request.data
        )
        if serializer.is_valid():
            serializer.save()
...
def edit_leave_administrators_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Administrators does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Leave Administrators deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
        serializer = serializers.LeaveAdministratorsSerializers(
            complain, data=request.data
        )
        if serializer.is_valid():
            serializer.save()
...
def edit_leave_migration_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Migration does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Leave Migration deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
...
def edit_restricted_holiday_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Restricted Holiday does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Restricted Holiday deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
        serializer = serializers.RestrictedHolidaySerializers(
            complain, data=request.data
        )
        if serializer.is_valid():
            serializer.save()
...
def edit_closed_holiday_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Closed Holiday does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Closed Holiday deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
...
def edit_vacation_holiday_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Vacation Holiday does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
        return Response(
            {"message": "Vacation Holiday deleted"}, status=status.HTTP_200_OK
        )
    elif request.method == "PUT":
...
def edit_leave_offline_api(request, c_id):
    try:
        ...
        return Response(
            {"message": "The Leave Offline does not exist"},
            status=status.HTTP_404_NOT_FOUND,
        )
    if request.method == "DELETE":
        obj.delete()
