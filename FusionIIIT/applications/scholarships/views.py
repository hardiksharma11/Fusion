    ...
    if request.method == "POST":
        if "Submit" in request.POST:
            ...
            if batch == "all":
                ...
                query = reduce(
                    or_, (Q(id__id__startswith=batch) for batch in active_batches)
                )
            ...
            pk = request.POST.get("id")
            award = (
                Proficiency_dm.objects.select_related("student", "award_id")
                .get(id=pk)
                .award_id
            )
            student_id = (
            ...
            year = datetime.datetime.now().year
            Proficiency_dm.objects.select_related("student", "award_id").filter(
                id=pk
            ).update(status="Accept")
            Previous_winner.objects.create(
                student=student_id, year=year, award_id=award
    ...
    for dates in release:
        if checkDate(dates.startdate, dates.enddate):
            if (
                ...
                and dates.batch == str(request.user.extrainfo.student)[0:4]
