...
class Constants:
    ...
    ATTEND_CHOICES = (("present", "present"), ("absent", "absent"))
...
class Student(models.Model):
    ...
    specialization = models.CharField(
        max_length=40, choices=Constants.MTechSpecialization, null=True, default=""
    )
...
class Curriculum_Instructor(models.Model):
    ...
    def __str__(self):
        return "{} - {}".format(
            self.curriculum_id.course_code, self.instructor_id.user.username
        )
...
class Holiday(models.Model):
    ...
    holiday_type = models.CharField(
        default="restricted", max_length=30, choices=Constants.HOLIDAY_TYPE
    )
