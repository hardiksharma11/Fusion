...
def main_page(request):
    ...
    if len(Income.objects.all()):
        ...
        if (
            min_date_ob["date_added__min"].year == pres_date.year
            and min_date_ob["date_added__min"].month < 4
        ):
...
def balanceSheet_table():
    ...
    result = (
        income_ob.values("source").annotate(amount=Sum("amount")).order_by("source")
    )
    income_dic = {}
    for each in result:
        income_dic[IncomeSource.objects.get(id=each["source"]).income_source] = each[
            "amount"
        ]
    ...
    result = (
        expenditure_ob.values("spent_on")
    ...
    for each in result:
        expenditure_dic[
            ExpenditureType.objects.get(id=each["spent_on"]).expenditure_type
        ] = each["amount"]
    ...
    pdf = render_to_pdf(
        "incomeExpenditure/balanceSheet_pdf.html",
        {
            "fixedDetails": fixed_attributes,
            "incomeDetails": income_dic,
            "incomeTypes": income_type_ob,
            "expenditureDetails": expenditure_dic,
            "expenditureTypes": expenditure_type_ob,
            "incomeSum": incomeSum,
    ...
    fin_year = (
        str(curr_year - 1) + "-" + str(curr_year)
        if timezone.now().date().month < 4
        else str(curr_year) + "-" + str(curr_year + 1)
    )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            response = HttpResponse(
                balance_sheet_ob.balanceSheet, content_type="application/pdf"
            )
            response["Content-Disposition"] = "attachment; filename=BalanceSheet.pdf"
            return response
        except Exception as e:
            return render(
                request, "../templates/incomeExpenditure/balanceSheetNotExists.html"
            )
...
def view_income_stats(request):
    if request.method == "POST":
        ...
        result = (
            temp.values("source").annotate(amount=Sum("amount")).order_by("-date_added")
        )
        income_labels = []
        income_data = []
        income_labels2 = []
...
def view_expenditure_stats(request):
    if request.method == "POST":
        ...
        # temp = Income.objects.filter(date_added__year = 2021)
        result = (
            temp.values("spent_on").annotate(amount=Sum("amount")).order_by("-amount")
        )
        expenditure_labels2 = []
        expenditure_data2 = []
        expenditure_data = []
        expenditure_labels = []
        for each in result:
            each["spent_on"] = ExpenditureType.objects.get(
                id=each["spent_on"]
            ).expenditure_type
        for each in result:
