...
class Cpda_Bills_Form(forms.Form):
    ...
    adjustment_amount = forms.IntegerField(
        label="Adjustment Amount", min_value=0, required=True
    )
    bills = forms.FileField(label="Bills")
    total_bills_amount = forms.IntegerField(
        label="Total Bills Amount", min_value=0, required=True
    )
...
class Ltc_Form(forms.Form):
    ...
    leave_end = forms.DateField(
        label="Leave End Date",
        widget=forms.TextInput(attrs={"placeholder": "YYYY-MM-DD"}),
    )
    family_departure_date = forms.DateField(
        label="Family Departure Date",
        widget=forms.TextInput(attrs={"placeholder": "YYYY-MM-DD"}),
    )
    ...
    leave_type = forms.ChoiceField(
        label="Whether L.T.C. is desired for going to home town or elsewhere?",
        choices=Constants.LTC_TYPE,
    )
    address_during_leave = forms.CharField(
        label="Address During Leave", widget=forms.TextInput
    )
    phone_number = forms.CharField(label="Phone Number", required=True)
    travel_mode = forms.ChoiceField(
        label="Mode of Travel", choices=Constants.LTC_TRAVEL, required=True
    )

    ltc_availed = forms.CharField(
        label="Details of family members that have already availed LTC in this block",
        widget=forms.TextInput,
        required=True,
    )
    ltc_to_avail = forms.CharField(
        label="Details of family members who will avail LTC: Name, Age",
        widget=forms.TextInput,
        required=True,
    )
    dependents = forms.CharField(
        label="Details of all dependents: Name, Age, Why fully dependent?",
        widget=forms.TextInput,
        required=True,
    )

    requested_advance = forms.IntegerField(
        label="Advance Requested", min_value=0, required=True
    )
...
class Assign_Form(forms.Form):
    ...
    reviewer_id = forms.CharField(
        label="ID",
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Username"}),
    )
    reviewer_id2 = forms.CharField(
        label="Reviewer ID 2",
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Username"}),
    )
    reviewer_id3 = forms.CharField(
        label="Reviewer ID 3",
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Username"}),
    )
    ...
    accept_status = forms.ChoiceField(
...
class Ltc_Eligible_Form(forms.Form):
    ...
    current_block_size = forms.IntegerField(
        label="Current LTC Block Size", required=True, min_value=0, initial=4
    )

    total_ltc_allowed = forms.IntegerField(
        label="Total LTC allowed", min_value=0, initial=2
    )
    hometown_ltc_allowed = forms.IntegerField(
        label="Hometown LTC allowed", min_value=0, initial=2
    )
    elsewhere_ltc_allowed = forms.IntegerField(
        label="Elsewhere LTC allowed", min_value=0, initial=1
    )

    hometown_ltc_availed = forms.IntegerField(
        label="Hometown LTC availed", min_value=0, initial=0
    )
    elsewhere_ltc_availed = forms.IntegerField(
        label="Elsewhere LTC availed", min_value=0, initial=0
    )
