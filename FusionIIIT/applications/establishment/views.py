...
def is_admin(request):
    ...
    return (
        request.user
        == Establishment_variables.objects.select_related("est_admin").first().est_admin
    )
...
def handle_cpda_admin(request):
    ...
    if "cpda_assign_form" in request.POST:
        try:
            ...
            application = Cpda_application.objects.select_related("applicant").get(
                id=app_id
            )
...
def handle_ltc_admin(request):
    ...
    if "ltc_assign_form" in request.POST:
        try:
            ...
            application = Ltc_application.objects.select_related("applicant").get(
                id=app_id
            )
    ...
    #     messages.success(request, 'Status updated successfully!')
        ...
        try:
            ...
            if Ltc_eligible_user.objects.filter(user=user_id).exists():
                messages.error(
                    request, "This user is already eligible for availing LTC"
                )
                return
        ...
        try:
            ...
            eligible_user = Ltc_eligible_user.objects.select_related("user").get(
                user=user_id
            )
        ...
        try:
            ...
            user_id = User.objects.get(username=username)
            if (
                not Ltc_eligible_user.objects.select_related("user")
                .filter(user=user_id)
                .exists()
            ):
                messages.error(
                    request, "This user already isn't eligible for availing LTC"
                )
                return
            ...
            messages.success(
                request, "User successfully removed from eligible LTC users"
            )
        except Exception as e:
            messages.error(request, e)
...
def generate_cpda_hod_admin_lists(request):
    ...
    try:
        ...
        unreviewed_apps = (
            Cpda_application.objects.select_related("applicant")
        ...
        for app in unreviewed_apps:
            ...
            if app.status == "requested":
                temp = Assign_Form(
                    initial={"assign_status": "requested", "app_id": app.id}
                )
            ...
            # if status is adjustments_pending:to_assign/reviewed
                ...
                temp = Assign_Form(
                    initial={"assign_status": "adjustments_pending", "app_id": app.id}
                )
                temp.fields["status"]._choices = [
...
def generate_cpda_admin_lists(request):
    ...
    try:
        to_review_apps = (
            ...
            .filter(
                (
                    Q(tracking_info__reviewer_id=request.user)
                    & Q(tracking_info__current_reviewer_id=1)
                )
                | (
                    Q(tracking_info__reviewer_id2=request.user)
                    & Q(tracking_info__current_reviewer_id=2)
                )
                | (
                    Q(tracking_info__reviewer_id3=request.user)
                    & Q(tracking_info__current_reviewer_id=3)
                )
            )
            .exclude(status="rejected")
            .exclude(status="finished")
        ...
        reviewed_apps = (
            ...
            .filter(
                Q(
                    tracking_info__reviewer_id=request.user,
                    tracking_info__current_reviewer_id__gte=2,
                )
                | Q(
                    tracking_info__reviewer_id2=request.user,
                    tracking_info__current_reviewer_id__gte=3,
                )
                | Q(
                    tracking_info__reviewer_id3=request.user,
                    tracking_info__current_reviewer_id__gte=4,
                )
            )
            .order_by("-request_timestamp")
        )
...
def generate_ltc_admin_lists(request):
    ...
    try:
        # only requested and adjustment_pending
        unreviewed_apps = (
            Ltc_application.objects.select_related("applicant")
        ...
        current_eligible_users = Ltc_eligible_user.objects.select_related(
            "user"
        ).order_by("user")
        for user in current_eligible_users:
            temp = Ltc_Eligible_Form(
                initial={
                    "username": user.user,
...
def handle_cpda_eligible(request):
    ...
    if "cpda_request" in request.POST:
        try:
            ...
            if (
                emp_consultancy_projects.objects.filter(user=request.user).first()
                is not None
            ):
                pf_number = (
                    emp_consultancy_projects.objects.filter(user=request.user)
                    .first()
                    .pf_no
                )
            else:
            ...
            # finding HR Admin
            hr_admin = (
                Establishment_variables.objects.select_related("est_admin")
                .first()
                .est_admin
            )
            # print(hr_admin)
        ...
        try:
            ...
            Cpda_bill.objects.create(application_id=app_id, bill=upload_file)
            bills_attached = 1
            timestamp = datetime.now()
        ...
        try:
            ...
            if application.tracking_info.current_reviewer_id == 3:
                ...
                if application.status == "adjustments_pending":
                    ...
                    if (
                        application.total_bills_amount - application.requested_advance
                    ) > 0:
                        cpda_bal.cpda_balance = cpda_bal.cpda_balance - (
                            application.total_bills_amount
                            - application.requested_advance
                        )
                    else:
                        cpda_bal.cpda_balance = cpda_bal.cpda_balance + (
                            application.total_bills_amount
                    ...
                    cpda_bal.cpda_balance = (
                        cpda_bal.cpda_balance - application.requested_advance
                    )
    ...
    if "ltc_request" in request.POST:
        try:
            ...
            if (
                ...
                messages.error(
                    request, "Please fill EIS Forms(Consultancy Project) first."
                )
                return
            ...
            ret = relativedelta(
                datetime.today().date(), eligible_ltc_user.date_of_joining
            )
            ...
            while 1:
                ...
                ltc_availed = Ltc_availed.objects.create(
                    ltc=application, name=name, age=age
                )
            ...
            while 1:
                ...
                dependent = Dependent.objects.create(
                    ltc=application, name=name, age=age, depend=depend
                )
            ...
            track = Ltc_tracking.objects.create(
                application=application, review_status="to_assign"
            )
            # add notif here
            messages.success(request, "Request sent successfully!")
...
def handle_appraisal(request):
    ...
    if "appraisal_request" in request.POST:
        try:
            ...
            conferences_meeting_attended = request.POST.get(
                "conferences_meeting_attended"
            )
            conferences_meeting_organized = request.POST.get(
                "conferences_meeting_organized"
            )
            ...
            while 1:
                ...
                tutorial_hrs_wk = request.POST.get(
                    "Tutorial (Hours/week)1.1.2" + str(count)
                )
            ...
            while 1:
                ...
                activity_type = request.POST.get(
                    "Type of Activity" + "1.1.3" + str(count)
                )
            ...
            appraisal_request = AppraisalRequest.objects.create(
                appraisal=application, hod=hod, director=director
            )
    ...
    if "director_appraisal_review" in request.POST:
        try:
            ...
            request_object = AppraisalRequest.objects.select_related(
                "appraisal"
            ).filter(appraisal=application)
            appraisal_track = request_object[0]
...
def generate_cpda_eligible_lists(request):
    ...
    try:
        ...
        if (
            ...
            pf_number = (
                emp_consultancy_projects.objects.filter(user=request.user).first().pf_no
            )
        ...
        block_period = (
            str(2019 + int((np.ceil((today_date.year - 2018) / 3) - 1)) * 3)
            + "-"
            + str(2018 + int(np.ceil((today_date.year - 2018) / 3)) * 3)
        )
        hod = is_hod(request)
...
def generate_ltc_eligible_lists(request):
    ...
    try:
        ltc_info = {}
        ltc_queryset = Ltc_eligible_user.objects.select_related("user").filter(
            user=request.user
        )
        ...
        if ltc_info["eligible"]:
            ...
            ltc_info["hometown_ltc_remaining"] = (
                ltc_queryset.first().hometown_ltc_remaining()
            )
            ltc_info["elsewhere_ltc_remaining"] = (
                ltc_queryset.first().elsewhere_ltc_remaining()
            )
            ...
            availed_active = Ltc_availed.objects.filter(
                ltc__applicant=request.user
            ).filter(ltc__status="requested")
            to_avail_active = Ltc_to_avail.objects.filter(
                ltc__applicant=request.user
            ).filter(ltc__status="requested")
            depend_active = Dependent.objects.filter(
                ltc__applicant=request.user
            ).filter(ltc__status="requested")
            availed_archived = Ltc_availed.objects.filter(
                ltc__applicant=request.user
            ).exclude(ltc__status="requested")
            to_avail_archived = Ltc_to_avail.objects.filter(
                ltc__applicant=request.user
            ).exclude(ltc__status="requested")
            depend_archived = Dependent.objects.filter(
                ltc__applicant=request.user
            ).exclude(ltc__status="requested")
            pf_number = 1234
            if (
        ...
        pf_number = 1234
        to_review_apps = (
            Ltc_application.objects.filter(tracking_info__reviewer_id=request.user)
            .filter(status="requested")
            .filter(tracking_info__review_status="under_review")
            .order_by("-request_timestamp")
        )
        ...
        availed_review = (
            Ltc_availed.objects.filter(ltc__tracking_info__reviewer_id=request.user)
            .filter(ltc__status="requested")
            .filter(ltc__tracking_info__review_status="under_review")
        )
        to_avail_review = (
            Ltc_to_avail.objects.filter(ltc__tracking_info__reviewer_id=request.user)
            .filter(ltc__status="requested")
            .filter(ltc__tracking_info__review_status="under_review")
        )
        depend_review = (
            Dependent.objects.filter(ltc__tracking_info__reviewer_id=request.user)
            .filter(ltc__status="requested")
            .filter(ltc__tracking_info__review_status="under_review")
        )
        for app in to_review_apps:
...
def generate_appraisal_lists(request):
    ...
    try:
        ...
        consultancy_projects = emp_consultancy_projects.objects.filter(
            user=request.user
        )
        ...
        active_apps = (
            Appraisal.objects.select_related("applicant")
            .filter(applicant=request.user)
        ...
        request_active = (
            AppraisalRequest.objects.select_related("appraisal")
            .filter(appraisal__applicant=request.user)
            .filter(appraisal__status="requested")
        )
        request_archived = (
            AppraisalRequest.objects.select_related("appraisal")
            .filter(appraisal__applicant=request.user)
            .exclude(appraisal__status="requested")
        )
        new_courses_offered = NewCoursesOffered.objects.filter(
            appraisal__applicant=request.user
        )
        new_courses_material = NewCourseMaterial.objects.filter(
            appraisal__applicant=request.user
        )
...
def generate_appraisal_lists_hod(request):
    ...
    try:
        ...
        review_apps_hod = (
            ...
            .filter(status_hod="pending")
            .order_by("-request_timestamp")
        )
        reviewed_apps_hod = (
            AppraisalRequest.objects.select_related("appraisal")
            .filter(hod=request.user)
            .exclude(status_hod="pending")
            .order_by("-request_timestamp")
        )
...
def generate_appraisal_lists_director(request):
    ...
    try:
        response = {}
        review_apps_director = (
            AppraisalRequest.objects.select_related("appraisal")
            .filter(director=request.user)
            .filter(status_director="pending")
            .exclude(status_hod="pending")
            .order_by("-request_timestamp")
        )
        reviewed_apps_director = (
            AppraisalRequest.objects.select_related("appraisal")
            .filter(director=request.user)
            .exclude(status_director="pending")
            .order_by("-request_timestamp")
        )
        course_objects_all = Curriculum_Instructor.objects.all()
        consultancy_projects_all = emp_consultancy_projects.objects.all()
...
def generate_appraisal_lists_admin(request):
    ...
    try:
        response = {}
        appraisal_requests = AppraisalRequest.objects.exclude(
    ...
    try:
        ...
        for obj in consultancy_projects:
            ...
            if (obj.start_date is not None) and (obj.end_date is not None):
                if (
                    start_date <= obj.start_date <= end_date
                    or start_date <= obj.end_date <= end_date
                ):
        ...
        for obj in research_projects:
            if (
                (obj.start_date is not None)
                and (obj.finish_date is not None)
                and (obj.date_submission is not None)
            ):
                if (
                    start_date <= obj.start_date <= end_date
...
def appraisal(request):
    ...
    if (
        ...
        and "filter_eis_details" in request.POST
    ):
