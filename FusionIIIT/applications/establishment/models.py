...
class Constants:
    ...
    LTC_TYPE = (("hometown", "Home Town"), ("elsewhere", "Elsewhere"))
    LTC_TRAVEL = (("rail", "Rail"), ("road", "Road"))
    LTC_LEAVE = (("yes", "Yes"), ("no", "No"))
...
class Cpda_tracking(models.Model):
    ...
    reviewer_id = models.ForeignKey(
        User, related_name="reviewer1", null=True, blank=True, on_delete=models.CASCADE
    )
    reviewer_id2 = models.ForeignKey(
        User, related_name="reviewer2", null=True, blank=True, on_delete=models.CASCADE
    )
    reviewer_id3 = models.ForeignKey(
        User, related_name="reviewer3", null=True, blank=True, on_delete=models.CASCADE
    )
    current_reviewer_id = models.IntegerField(blank=True, default=1)
    reviewer_design = models.ForeignKey(
        Designation,
        related_name="desig1",
        null=True,
        blank=True,
    ...
    reviewer_design2 = models.ForeignKey(
        Designation,
        related_name="desig2",
        null=True,
        blank=True,
    ...
    reviewer_design3 = models.ForeignKey(
        Designation,
        related_name="desig3",
        null=True,
        blank=True,
    ...
    review_status = models.CharField(
        max_length=20, null=True, choices=Constants.REVIEW_STATUS
    )
...
class Cpda_bill(models.Model):
    application = models.ForeignKey(
        Cpda_application, on_delete=models.CASCADE, null=True
    )
...
class Ltc_application(models.Model):
    ...
    purpose = models.CharField(max_length=500, default="", blank=True)
    is_hometown_or_elsewhere = models.CharField(
        choices=Constants.LTC_TYPE, max_length=50
    )
...
class Ltc_availed(models.Model):
    ltc = models.ForeignKey(
        Ltc_application, related_name="ltcAvailed", on_delete=models.CASCADE
    )
...
class Ltc_to_avail(models.Model):
    ltc = models.ForeignKey(
        Ltc_application, related_name="ltcToAvail", on_delete=models.CASCADE
    )
    name = models.CharField(max_length=30)
    age = models.IntegerField(blank=True, null=True)
...
class Dependent(models.Model):
    ltc = models.ForeignKey(
        Ltc_application, related_name="Dependent", on_delete=models.CASCADE
    )
    name = models.CharField(max_length=30)
    age = models.IntegerField(blank=True, null=True)
...
class Ltc_eligible_user(models.Model):
    ...
    def total_ltc_remaining(self):
        x = (
            self.hometown_ltc_allowed
            - self.hometown_ltc_availed
            + self.elsewhere_ltc_allowed
    ...
    def hometown_ltc_remaining(self):
        x = self.hometown_ltc_allowed - self.hometown_ltc_availed
        x = max(x, 0)
        return x

    def elsewhere_ltc_remaining(self):
        x = self.elsewhere_ltc_allowed - self.elsewhere_ltc_availed
        x = max(x, 0)
        return x
...
class Appraisal(models.Model):
    ...
    applicant = models.ForeignKey(
        User, related_name="all_appraisals", on_delete=models.CASCADE
    )
    designation = models.ForeignKey(
        Designation, null=True, related_name="desig", on_delete=models.SET_NULL
    )
    ...
    other_research_element = models.CharField(
        max_length=200, blank=True, null=True, default=""
    )
    publications = models.CharField(max_length=200, blank=True, null=True, default="")
    conferences_meeting_attended = models.CharField(
        max_length=200, blank=True, null=True, default=""
    )
    conferences_meeting_organized = models.CharField(
        max_length=200, blank=True, null=True, default=""
    )
    ...
    faculty_comments = models.CharField(
        max_length=200, blank=True, null=True, default=""
    )
...
class CoursesInstructed(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal, related_name="applicant_courses", on_delete=models.CASCADE
    )
    ...
    def __str__(self):
        return (
            str(self.appraisal.applicant.username)
            + " - Course Name: "
...
class NewCoursesOffered(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal,
        related_name="applicants_offered_new_courses",
        on_delete=models.CASCADE,
    )
    course_name = models.CharField(max_length=30)
    course_num = models.IntegerField(blank=True, null=True)
...
class NewCourseMaterial(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal,
        related_name="applicant_new_courses_material",
        on_delete=models.CASCADE,
    )
    course_name = models.CharField(max_length=30)
    course_num = models.IntegerField(blank=True, null=True)
...
class ThesisResearchSupervision(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal, related_name="applicants_supervised_stud", on_delete=models.CASCADE
    )
    ...
    co_supervisors = models.ForeignKey(
        User,
        related_name="all_supervisors",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
...
class SponsoredProjects(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal, related_name="applicant_sponsored_projects", on_delete=models.CASCADE
    )
    project_title = models.CharField(max_length=30)
    sponsoring_agency = models.CharField(max_length=30)
    funding = models.IntegerField()
    duration = models.IntegerField()
    co_investigators = models.ForeignKey(
        User,
        related_name="all_co_investigators",
        on_delete=models.CASCADE,
        blank=True,
        null=True,
    )
...
class AppraisalRequest(models.Model):
    ...
    appraisal = models.ForeignKey(
        Appraisal, related_name="appraisal_tracking", on_delete=models.CASCADE
    )
    hod = models.ForeignKey(
        User, related_name="hod", on_delete=models.CASCADE, null=True
    )
    director = models.ForeignKey(
        User, related_name="director", on_delete=models.CASCADE, null=True
    )
    ...
    status_hod = models.CharField(
        max_length=20, default="pending", choices=Constants.STATUS
    )
    status_director = models.CharField(
        max_length=20, default="pending", choices=Constants.STATUS
    )
...
class AppraisalAdministrators(models.Model):
    ...
    user = models.OneToOneField(
        User, related_name="apprasial_admins", on_delete=models.CASCADE
    )
    authority = models.ForeignKey(
        Designation,
        null=True,
        related_name="sanc_authority_of_ap",
        on_delete=models.SET_NULL,
    )
    officer = models.ForeignKey(
        Designation,
        null=True,
        related_name="sanc_officer_of_ap",
        on_delete=models.SET_NULL,
    )
