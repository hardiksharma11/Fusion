...
def dep_main(request):
    ...
    fac_view = request.user.holds_designations.filter(
        designation__name="faculty"
    ).exists()
    student = request.user.holds_designations.filter(
    ...
    if request.method == "POST":
        ...
        obj_sprequest, created_object = SpecialRequest.objects.get_or_create(
...
def all_students(request, bid):
    ...
    if int(bid) == 1:
        ...
        student_list1 = (
            Student.objects.order_by("id")
            .filter(
                programme="PhD", id__user_type="student", id__department__name="CSE"
            )
            .select_related("id")
        )
        paginator = Paginator(student_list1, 25, orphans=5)
        ...
        student_list1 = (
            Student.objects.order_by("id")
            .filter(
                programme="PhD", id__user_type="student", id__department__name="ECE"
            )
            .select_related("id")
        )
        paginator = Paginator(student_list1, 25, orphans=5)
        ...
        student_list1 = (
            Student.objects.order_by("id")
            .filter(programme="PhD", id__user_type="student", id__department__name="ME")
            .select_related("id")
        )
        paginator = Paginator(student_list1, 25, orphans=5)
        page_number = request.GET.get("page")
        ...
        student_list1 = (
            Student.objects.order_by("id")
            .filter(programme="PhD", id__user_type="student", id__department__name="SM")
            .select_related("id")
        )
        paginator = Paginator(student_list1, 25, orphans=5)
        page_number = request.GET.get("page")
...
def approved(request):
    ...
    if request.method == "POST":
        ...
        SpecialRequest.objects.filter(id=request_id).update(
            status="Approved", remarks=remark
        )
    request.method = ""
