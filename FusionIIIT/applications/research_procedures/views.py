...
def patent_registration(request):
    ...
    if request.method == "POST":
        if (
            ("ipd_form_file" in request.FILES)
            and ("project_details_file" in request.FILES)
            and ("title" in request.POST)
        ):
            if user_extra_info.user_type == "faculty":
                ...
                if ipd_form_pdf.name.endswith(".pdf"):
                    ...
                    ipd_form_pdf_name = file_system.save(
                        ipd_form_pdf.name, ipd_form_pdf
                    )
                ...
                if project_details_pdf.name.endswith(".pdf"):
                    ...
                    file_system = FileSystemStorage()
                    project_details_pdf_name = file_system.save(
                        project_details_pdf.name, project_details_pdf
                    )
                    patent.project_details_file = file_system.url(
                        project_details_pdf_name
                    )
                ...
                dean_rspc_user = HoldsDesignation.objects.get(
                    designation=Designation.objects.filter(name="dean_rspc").first()
                ).working
...
def patent_status_update(request):
    ...
    if request.method == "POST":
        if user_designations.exists():
            if (
                user_designations.first().designation.name == "dean_rspc"
                ...
                research_procedures_notif(
                    dean_rspc_user, patent.faculty_id.user, request.POST.get("status")
                )
            else:
...
def project_insert(request):
    ...
    if (
        ...
        try:
            research_project.date_submission = datetime.datetime.strptime(
                x, "%B %d, %Y"
            )
        except:
            research_project.date_submission = datetime.datetime.strptime(
