    ...
    if editable and request.method == "POST":
        ...
        if "extracurricularsubmit" in request.POST:
            ...
            if form.is_valid():
                ...
                date_earned = form.cleaned_data["date_earned"]
                extracurricular_obj = Extracurricular.objects.create(
                    unique_id=student,
...
def contextfacultymanage(request, user, profile):
    ...
    publications = emp_research_papers.objects.filter(
        pf_no=profile.id, rtype="Journal"
    ).order_by("-date_entry")
    ...
    conferences = emp_research_papers.objects.filter(
        pf_no=profile.id, rtype="Conference"
    ).order_by("-date_entry")
    ...
    research_projects = emp_research_projects.objects.filter(pf_no=profile.id).order_by(
        "-date_entry"
    )
    ...
    techtransfer = emp_techtransfer.objects.filter(pf_no=profile.id).order_by(
        "-date_entry"
    )
    ...
    thesis = emp_mtechphd_thesis.objects.filter(
        pf_no=profile.id, degree_type=1
    ).order_by("-date_entry")
    ...
    phdthesis = emp_mtechphd_thesis.objects.filter(
        pf_no=profile.id, degree_type=2
    ).order_by("-date_entry")
    ...
    foreign_visits = emp_visits.objects.filter(pf_no=profile.id, v_type=2).order_by(
        "-start_date"
    )
    ...
    indian_visits = emp_visits.objects.filter(pf_no=profile.id, v_type=1).order_by(
        "-start_date"
    )
    ...
    events = emp_event_organized.objects.filter(pf_no=profile.id).order_by(
        "-date_entry"
    )
    ...
    confs = emp_confrence_organised.objects.filter(pf_no=profile.id).order_by(
        "-date_entry"
    )
