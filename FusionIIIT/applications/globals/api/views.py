...
def dashboard(request):
    ...
    designation_id = [
        designation for designations in designation_list for designation in designations
    ]
    designation_info = []
...
def profile(request, username=None):
    ...
    if profile["user_type"] == "student":
        ...
        education = serializers.EducationSerializer(
            student.education_set.all(), many=True
        ).data
        course = serializers.CourseSerializer(student.course_set.all(), many=True).data
        experience = serializers.ExperienceSerializer(
            student.experience_set.all(), many=True
        ).data
        project = serializers.ProjectSerializer(
            student.project_set.all(), many=True
        ).data
        achievement = serializers.AchievementSerializer(
            student.achievement_set.all(), many=True
        ).data
        publication = serializers.PublicationSerializer(
            student.publication_set.all(), many=True
        ).data
        patent = serializers.PatentSerializer(student.patent_set.all(), many=True).data
        current = serializers.HoldsDesignationSerializer(
            user.current_designation.all(), many=True
        ).data
        resp = {
            "user": user_detail,
            "profile": profile,
...
def profile_update(request):
    ...
    if current:
        ...
        if "education" in request.data:
            ...
            serializer = serializers.ExtraInfoSerializer(
                profile, data=request.data["profilesubmit"], partial=True
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            serializer = serializers.AchievementSerializer(
                data=request.data["achievementsubmit"]
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            serializer = serializers.PublicationSerializer(
                data=request.data["publicationsubmit"]
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            serializer = serializers.ProjectSerializer(
                data=request.data["projectsubmit"]
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
            ...
            serializer = serializers.ExperienceSerializer(
                data=request.data["experiencesubmit"]
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_200_OK)
...
def profile_delete(request, id):
    ...
    if "deleteskill" in request.data:
        try:
            ...
            return Response(
                {"error": "Skill does not exist"}, status=status.HTTP_400_BAD_REQUEST
            )
        skill.delete()
        return Response(
        ...
        try:
            ...
            return Response(
                {"error": "Education does not exist"},
        ...
        achievement.delete()
        return Response(
            {"message": "Achievement deleted successfully"}, status=status.HTTP_200_OK
        )
        ...
        publication.delete()
        return Response(
            {"message": "Publication deleted successfully"}, status=status.HTTP_200_OK
        )
        ...
        try:
            ...
            return Response(
                {"error": "Patent does not exist"}, status=status.HTTP_400_BAD_REQUEST
            )
        patent.delete()
        return Response(
            {"message": "Patent deleted successfully"}, status=status.HTTP_200_OK
        )
...
def NotificationRead(request):
    try:
        ...
        notification = get_object_or_404(
            Notification, recipient=request.user, id=notifId
        )
        notification.mark_as_read()
        response = {"message": "notfication successfully marked as seen."}
        return Response(response, status=status.HTTP_200_OK)
    except:
        response = {"error": "Failed, notification is not marked as seen."}
