...
def compounder_view(request):
    ...
    if usertype == "compounder":
        if request.method == "POST":
            ...
            users = ExtraInfo.objects.select_related("user", "department").filter(
                user_type="student"
            )
            stocks = Stock.objects.all()
            days = Constants.DAYS_OF_WEEK
            schedule = (
                Schedule.objects.select_related("doctor_id").all().order_by("doctor_id")
            )
            expired = (
                Expiry.objects.select_related("medicine_id")
                .filter(expiry_date__lt=datetime.now(), returned=False)
            ...
            medicines_presc = Prescribed_medicine.objects.select_related(
                "prescription_id",
                "prescription_id__user_id",
                "prescription_id__user_id__user",
                "prescription_id__user_id__department",
                "prescription_id__doctor_id",
                "prescription_id__appointment",
                "prescription_id__appointment__user_id",
                "prescription_id__appointment__user_id__user",
                "prescription_id__appointment__user_id__department",
                "prescription_id__appointment__doctor_id",
                "prescription_id__appointment__schedule",
                "prescription_id__appointment__schedule__doctor_id",
                "medicine_id",
            ).all()
            if count:
...
def student_view(request):
    ...
    if usertype == "student" or usertype == "faculty" or usertype == "staff":
        if request.method == "POST":
            ...
            medicines = Prescribed_medicine.objects.select_related(
                "prescription_id",
                "prescription_id__user_id",
                "prescription_id__user_id__user",
                "prescription_id__user_id__department",
...
def schedule_entry(request):
    ...
    excel = xlrd.open_workbook(
        os.path.join(os.getcwd(), "dbinsertscripts/healthcenter/Doctor-Schedule.xlsx")
    )
...
def doctor_entry(request):
    ...
    """
    excel = xlrd.open_workbook(
        os.path.join(os.getcwd(), "dbinsertscripts/healthcenter/Doctor-List.xlsx")
    )
    z = excel.sheet_by_index(0)
    ...
    for i in range(1, 5):
        try:
            ...
            u = Doctor.objects.create(
                doctor_name=name, doctor_phone=phone, specialization=spl
            )
            print("Doctor done -> ")
        except Exception as e:
            print(e)
...
def compounder_entry(request):
    ...
    """
    excel = xlrd.open_workbook(
        os.path.join(os.getcwd(), "dbinsertscripts/healthcenter/Compounder-List.xlsx")
    )
    z = excel.sheet_by_index(0)
