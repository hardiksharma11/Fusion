...
def student_request_api(request):
    ...
    if usertype == "student" or usertype == "faculty" or usertype == "POST":
        if "ambulancerequest" in request.data and request.method == "POST":
            ...
            request.data["user_id"] = get_object_or_404(
            ...
            try:
                ...
                return Response(
                    {"message": "Please enter ambulance id"},
            ...
            request.data["schedule"] = get_object_or_404(
                Schedule, doctor_id=request.data["doctor_id"], day=day
            ).id
            comp_id = ExtraInfo.objects.filter(user_type="compounder")
...
def student_view_api(request):  # student view starts here
    ...
    if usertype == "student" or usertype == "faculty" or usertype == "staff":
        ...
        hospitals = serializers.HospitalAdmitSerializer(
            Hospital_admit.objects.filter(user_id=user_id).order_by("-admission_date"),
            many=True,
        ).data
        appointments = serializers.AppointmentSerializer(
            Appointment.objects.filter(user_id=user_id).order_by("-date"), many=True
        ).data
        ambulances = serializers.AmbulanceRequestSerializer(
            Ambulance_request.objects.filter(user_id=user_id).order_by("-date_request"),
            many=True,
        ).data
        prescription = serializers.PrescriptionSerializer(
            Prescription.objects.filter(user_id=user_id).order_by("-date"), many=True
        ).data
        medicines = serializers.PrescribedMedicineSerializer(
            Prescribed_medicine.objects.all(), many=True
        ).data
        complaints = serializers.ComplaintSerializer(
            Complaint.objects.filter(user_id=user_id).order_by("-date"), many=True
        ).data
        days = Constants.DAYS_OF_WEEK
        schedule = serializers.ScheduleSerializer(
            Schedule.objects.all().order_by("doctor_id"), many=True
        ).data
        doctors = serializers.DoctorSerializer(
            Doctor.objects.filter(active=True), many=True
        ).data
...
def compounder_request_api(request):
    ...
    if usertype == "compounder":
        if "doctoradd" in request.data and request.method == "POST":
            ...
            serializer = serializers.DoctorSerializer(
            ...
            serializer = serializers.HospitalAdmitSerializer(
                Ha, data=request.data, partial=True
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            ...
            request.data["medicine_id"] = (
                Stock.objects.get(medicine_name=request.data["medicine_name"])
            ).id
            ...
            serializer = serializers.StockSerializer(
                stock,
                data={"quantity": quantity, "threshold": request.data["threshold"]},
                partial=True,
            )
            if serializer.is_valid():
                serializer.save()
            ...
            with transaction.atomic():
                ...
                if stock > quantity:
                    ...
                    serializer = serializers.PrescribedMedicineSerializer(
                        data=request.data
                    )
                    if serializer.is_valid():
                        serializer.save()
            ...
            try:
                ...
                return Response(
                    {"message": "Please enter valid complaint id"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            try:
            ...
            serializer = serializers.ComplaintSerializer(
                complain, data=request.data, partial=True
            )
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
...
def compounder_view_api(request):  # compounder view starts here
    usertype = ExtraInfo.objects.get(user=request.user).user_type
    if usertype == "compounder":
        all_complaints = serializers.ComplaintSerializer(
            Complaint.objects.all(), many=True
        ).data
        all_hospitals = serializers.HospitalAdmitSerializer(
            Hospital_admit.objects.all().order_by("-admission_date"), many=True
        ).data
        hospitals_list = serializers.HospitalSerializer(
            Hospital.objects.all().order_by("hospital_name"), many=True
        ).data
        all_ambulances = serializers.AmbulanceRequestSerializer(
            Ambulance_request.objects.all().order_by("-date_request"), many=True
        ).data
        appointments_today = serializers.AppointmentSerializer(
            Appointment.objects.filter(date=datetime.now()).order_by("date"), many=True
        ).data
        appointments_future = serializers.AppointmentSerializer(
            Appointment.objects.filter(date__gt=datetime.now()).order_by("date"),
            many=True,
        ).data
        ...
        expired = serializers.ExpirySerializer(
            Expiry.objects.filter(
                expiry_date__lt=datetime.now(), returned=False
            ).order_by("expiry_date"),
            many=True,
        ).data
        live_meds = serializers.ExpirySerializer(
            Expiry.objects.filter(returned=False).order_by("quantity"), many=True
        ).data
        count = Counter.objects.all()
        presc_hist = serializers.PrescriptionSerializer(
            Prescription.objects.all().order_by("-date"), many=True
        ).data
        medicines_presc = serializers.PrescribedMedicineSerializer(
            Prescribed_medicine.objects.all(), many=True
        ).data
        ...
        schedule = serializers.ScheduleSerializer(
            Schedule.objects.all().order_by("day", "doctor_id"), many=True
        ).data
        doctors = serializers.DoctorSerializer(
            Doctor.objects.filter(active=True).order_by("id"), many=True
        ).data
        ...
        resp = {"message": "invalid request"}
        return Response(
            data=resp, status=status.HTTP_404_NOT_FOUND
        )  # compounder view ends
