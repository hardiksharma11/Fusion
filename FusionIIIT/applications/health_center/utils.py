...
def compounder_view_handler(request):
    ...
    if "feed_com" in request.POST:
        ...
        Complaint.objects.select_related(
            "user_id", "user_id__user", "user_id__department"
        ).filter(id=pk).update(feedback=feedback)
        data = {"feedback": feedback}
        return JsonResponse(data)

    elif "end" in request.POST:
        pk = request.POST.get("id")
        Ambulance_request.objects.select_related(
            "user_id", "user_id__user", "user_id__department"
        ).filter(id=pk).update(end_date=datetime.now())
        amb = Ambulance_request.objects.select_related(
            "user_id", "user_id__user", "user_id__department"
        ).filter(id=pk)
        for f in amb:
            dateo = f.end_date
    ...
    # return expired medicine and update db
        ...
        pk = request.POST.get("id")
        Expiry.objects.select_related("medicine_id").filter(id=pk).update(
            returned=True, return_date=datetime.now()
        )
    ...
    # discharge patients
        ...
        Hospital_admit.objects.select_related(
            "user_id", "user_id__user", "user_id__department", "doctor_id"
        ).filter(id=pk).update(discharge_date=datetime.now())
        hosp = Hospital_admit.objects.select_related(
            "user_id", "user_id__user", "user_id__department", "doctor_id"
        ).filter(id=pk)
        for f in hosp:
    ...
    # edit schedule for doctors
        ...
        schedule = Schedule.objects.select_related("doctor_id").filter(
            doctor_id=doctor, day=day
        )
        ...
        if schedule.count() == 0:
            ...
            Schedule.objects.select_related("doctor_id").filter(
                doctor_id=doctor_id, day=day
            ).update(room=room)
            Schedule.objects.select_related("doctor_id").filter(
                doctor_id=doctor_id, day=day
            ).update(from_time=time_in)
            Schedule.objects.select_related("doctor_id").filter(
                doctor_id=doctor_id, day=day
            ).update(to_time=time_out)
        data = {"status": 1}
        return JsonResponse(data)
    ...
    # remove schedule for a doctor
        ...
        Stock.objects.create(
            medicine_name=medicine, quantity=quantity, threshold=threshold
        )
        ...
        for med in user_medicine:
            list.append(
                {
                    "medicine": med.medicine_id.medicine_name,
        ...
        for s in schs:
            list.append(
                {
                    "medicine": s.medicine_id.medicine_name,
                    "quantity": s.quantity,
                    "days": s.days,
                    "times": s.times,
                }
            ...
            list.append(
                {
                    "room": s.room,
                    "id": s.id,
                    "doctor": s.doctor_id.doctor_name,
        ...
        query = Medicine.objects.select_related(
            "patient", "patient__user", "patient__department"
        ).objects.filter(patient=user)
        prescribe = (
            Prescription.objects.select_related(
                ...
                "doctor_id",
                "appointment",
                "appointment__user_id",
                "appointment__user_id__user",
                "appointment__user_id__department",
                "appointment__doctor_id",
                "appointment__schedule",
                "appointment__schedule__doctor_id",
            )
            .objects.all()
            .last()
        )
        for medicine in query:
            ...
            today = datetime.now()
            expiry = (
                Expiry.objects.select_related("medicine_id")
                .filter(
                    medicine_id=medicine_id,
                    quantity__gt=0,
                    returned=False,
            ...
            if stock > quantity:
                for e in expiry:
                    ...
                    if q > quantity:
                        ...
                        qty = qty - quantity
                        Stock.objects.filter(medicine_name=medicine_id).update(
                            quantity=qty
                        )
                        break
                    else:
        ...
        app = Appointment.objects.select_related(
            ...
            "schedule",
            "schedule__doctor_id",
        ).filter(user_id=user_id, date=datetime.now())
        if app:
            appointment = Appointment.objects.select_related(
                "user_id",
                "user_id__user",
                ...
                "schedule",
                "schedule__doctor_id",
            ).get(user_id=user_id, date=datetime.now())
        else:
            appointment = None
        Prescription.objects.create(
            user_id=user,
...
def student_view_handler(request):
    if "amb_submit" in request.POST:
        user_id = ExtraInfo.objects.select_related("user", "department").get(
            user=request.user
        )
        comp_id = ExtraInfo.objects.select_related("user", "department").filter(
        ...
        data = {"app_time": app_time, "dt": datei, "status": 1}
        ...
        doctor_id = request.POST.get("doctor")
        days = (
            Schedule.objects.select_related("doctor_id")
        ...
        Complaint.objects.create(
            user_id=user_id, complaint=feedback, date=datetime.now()
        )
        data = {"status": 1}
        return JsonResponse(data)
