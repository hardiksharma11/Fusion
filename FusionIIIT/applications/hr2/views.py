...
def service_book(request):
    ...
    lien_service_book = (
        ForeignService.objects.filter(extra_info=extra_info)
        .filter(service_type="LIEN")
        .order_by("-start_date")
    )
    deputation_service_book = (
        ForeignService.objects.filter(extra_info=extra_info)
        .filter(service_type="DEPUTATION")
        .order_by("-start_date")
    )
    other_service_book = (
        ForeignService.objects.filter(extra_info=extra_info)
    ...
    appraisal_form = EmpAppraisalForm.objects.filter(extra_info=extra_info).order_by(
        "-year"
    )
    pf = extra_info.id
    workAssignemnt = WorkAssignemnt.objects.filter(extra_info_id=pf).order_by(
        "-start_date"
    )
    ...
    conferences = emp_confrence_organised.objects.filter(pf_no=pf).order_by(
        "-date_entry"
    )
    ...
    context = {
        "lienServiceBooks": lien_service_book,
        "deputationServiceBooks": deputation_service_book,
        "otherServiceBooks": other_service_book,
