...
def libraryModule(request):
    ...
    library_issue_details_url = (
        "frmissuesofuser.aspx?title=Issue%20Details%20of%20The%20%20Members"
    )
    ...
    issue_request = requests.get(
        library_home_url + library_issue_details_url, cookies=home_request.cookies
    )
    ...
    Status = "Complete"
    memberid = ExtraInfo.objects.get(
        user=request.user
    ).id  # Userid of the member who logged in
    ...
    issue_result = requests.post(
        library_home_url + library_issue_details_url,
        cookies=home_request.cookies,
        data=formfields,
    )
    ...
    for div in soup_issue.find_all("div", {"id": "print13"}):
        if div.find_all("tr", {"class": ["GridItem", "GridAltItem"]}):
            ...
            for table_row in div.find_all("tr", {"class": ["GridItem", "GridAltItem"]}):
                new_row = {
                    str(index): {
                        "assn": table_row.contents[1].text,
    ...
    library_due_details_url = (
        "CircTotalFineUserWise.aspx?title=Over%20Due%20Details%20of%20MembersD"
    )
    due_request = requests.get(
        library_home_url + library_due_details_url, cookies=home_request.cookies
    )
    ...
    due_result = requests.post(
        library_home_url + library_due_details_url,
        cookies=home_request.cookies,
        data=formfields,
    )
    ...
    if request.method == "POST":
        ...
        catalogue_request = requests.get(
            library_home_url + catalogue_search_url, cookies=home_request.cookies
        )
        ...
        catalogue_result = requests.post(
            library_home_url + catalogue_search_result_url,
            cookies=home_request.cookies,
            data=formfields,
        )
        ...
        if text == "\nNo Result Found.\nNew Search":
            ...
            book_results = book_results.replace(
                "Copies Information Reserve Add Keyword(s) Add To Cart Content Author Info Book Info",
                "\n",
            )
    ...
    context = {
        "data1": issue_details,
        "due": due,
        "data2": due_details_non_returned,
        "data3": due_details_returned,
        "bookresults": book_results,
    }
