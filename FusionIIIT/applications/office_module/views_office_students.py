...
def getUniversalContext(
    ...
    budget_allotment = Club_info.objects.all().filter(
        alloted_budget=0, status="confirmed"
    )
...
def officeOfDeanStudents(request):
    ...
    return render(
        request,
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request,
            page=page,
            flag_dean_s=flag_dean_s,
...
def holdingMeeting(request):
    ...
    if date == "":
        ...
        given = datetime.datetime.strptime(
            date + " " + Time, "%Y-%m-%d %H:%M"
        ).timestamp()
        ...
        if time_diff < 0:
            ...
            p.save()
            success_msg = (
                "Meeting created successfully. Waiting for Suprintendent for the MOM"
            )
            Dean = HoldsDesignation.objects.get(
                designation=Designation.objects.filter(name="Dean_s")
            ).working
            Superintendent = HoldsDesignation.objects.filter(
                designation__name="Junior Superintendent"
            ).first()
    ...
    return render(
        ...
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request, page=1, success_msg=success_msg, err_msg=err_msg, flag_dean_s=True
        ),
    )
...
def hostelRoomAllotment(request):
    ...
    if hall_no == None:
        ...
        if (int(capacity.current_capacity) - int(num_students)) >= 0:
            ...
            p.save()
            capacity.current_capacity = int(capacity.current_capacity) - int(
                num_students
            )
            capacity.save()
...
def deleteHostelRoomAllotment(request):
    ...
    if id == None:
        ...
        capacity = hostel_capacity.objects.get(name=hall_no)
        if (
            int(capacity.current_capacity) + int(num_students)
        ...
        for i in total_students:
            if count + 1 <= num_students and str(i.hall_no) != "0":
                ...
                get_room = HallRoom.objects.get(
                    hall=hall_no, block_no=block, room_no=room
                )
...
def deleteAllHostelRoomAllotment(request):
    ...
    return render(
        ...
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request, page=7, success_msg=success_msg, flag_superintendent=True
        ),
    )
...
def budgetApproval(request):
    ...
    for i in range(len(id_r)):
        ...
        if int(budget) > avail_budget.avail_budget:
            ...
            Club_info_object = Club_info.objects.get(
                club_name=Club_budget_object.club.club_name
            )
            Club_info_object.spent_budget = (
                int(budget) - Club_info_object.spent_budget
            )  # (spentBudget + int(budget))
            Club_info_object.avail_budget = Club_info_object.avail_budget - int(
                budget
            )  # (availBudget - int(budget))
    ...
    return render(
        ...
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request, page=2, success_msg=success_msg, err_msg=err_msg, flag_dean_s=True
        ),
    )
...
def budgetRejection(request):
    ...
    return render(
        request,
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(request, page=2, success_msg=success_msg, flag_dean_s=True),
    )
...
def clubApproval(request):
    ...
    for i in range(len(id_r)):
        ...
        HoldsDesig = HoldsDesignation(
            user=co_ordinator, working=co_ordinator, designation=designation
        )
        HoldsDesig.save()
        HoldsDesig = HoldsDesignation(
            user=co_co, working=co_co, designation=designation1
        )
        HoldsDesig.save()
    ...
    return render(
        request,
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(request, page=5, success_msg=success_msg, flag_dean_s=True),
    )
...
def clubRejection(request):
    ...
    return render(
        request,
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(request, page=5, err_msg=err_msg, flag_dean_s=True),
    )
...
def sessionApproval(request):
    ...
    return render(
        ...
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request, page=11, success_msg=success_msg, flag_superintendent=True
        ),
    )
...
def budgetAllot(request):
    ...
    return render(
        ...
        "officeModule/officeOfDeanStudents/officeOfDeanStudents.html",
        getUniversalContext(
            request, page=10, success_msg=success_msg, flag_superintendent=True
        ),
    )
