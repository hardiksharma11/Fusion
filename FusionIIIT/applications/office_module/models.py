...
class Constants:
    ...
    DEPARTMENT = (("civil", "civil"), ("electrical", "electrical"))
    ...
    STATUS_CHOICES = (("Forward", "FORWARD"), ("Accept", "ACCEPT"))
    ...
    TICK_TYPE = (("NO", "YES"), ("NO", "NO"))

    PROJECT_OPERATED = (("PI", "Only by PI"), ("any", "Either PI or CO-PI"))

    TRAVEL_CHOICES = (("road", "ROAD"), ("rail", "RAIL"))
...
class Project_Registration(models.Model):
    ...
    agreement = models.CharField(
        choices=Constants.TICK_TYPE, max_length=20, default="NO"
    )
    amount_sanctioned = models.IntegerField(default=0)
    project_type = models.CharField(choices=Constants.PROJECT_TYPE, max_length=25)
    project_operated = models.CharField(
        choices=Constants.PROJECT_OPERATED, max_length=50, default="me"
    )
    remarks = models.CharField(max_length=200)
    fund_recieved_date = models.DateField(null=True, blank=True)
    HOD_response = models.CharField(
        choices=Constants.RESPONSE_TYPE1, max_length=10, default="Pending"
    )
    DRSPC_response = models.CharField(
        choices=Constants.RESPONSE_TYPE, max_length=10, default="Pending"
    )
...
class Filemovement(models.Model):
    ...
    sentby = models.ForeignKey(
        HoldsDesignation, on_delete=models.CASCADE, related_name="sent_by"
    )
    receivedby = models.ForeignKey(
        HoldsDesignation, on_delete=models.CASCADE, related_name="received_by"
    )
...
class apply_for_purchase(models.Model):
    indentor_name = models.ForeignKey(
        ExtraInfo, on_delete=models.CASCADE, related_name="indentor_name"
    )
...
class purchase_commitee(models.Model):
    local_comm_mem1 = models.ForeignKey(
        ExtraInfo, on_delete=models.CASCADE, related_name="local_comm_mem1"
    )
    local_comm_mem2 = models.ForeignKey(
        ExtraInfo, on_delete=models.CASCADE, related_name="local_comm_mem2"
    )
    local_comm_mem3 = models.ForeignKey(
        ExtraInfo, on_delete=models.CASCADE, related_name="local_comm_mem3"
    )
...
class LTC(models.Model):
    ...
    travel_mode = models.CharField(
        max_length=10, choices=Constants.TRAVEL_CHOICES, default="ROAD"
    )
...
class Teaching_credits1(models.Model):
    ...
    course1 = models.CharField(
        choices=Constants.TICK_TYPE, max_length=100, default="NO"
    )
...
class Lab(models.Model):
    ...
    day = models.CharField(
        max_length=10, choices=Constants.DAY_CHOICES, default="Monday"
    )
...
class TA_assign(models.Model):
    roll_no = models.ForeignKey(
        ExtraInfo, on_delete=models.CASCADE, related_name="TA_id"
    )
...
class Registrar_response(models.Model):
    track_id = models.ForeignKey(
        Tracking, on_delete=models.CASCADE, related_name="t_id"
    )
