...
from applications.globals.models import Designation, ExtraInfo, HoldsDesignation, User
...
from applications.eis.models import (
    emp_research_projects,
    emp_patents,
    emp_consultancy_projects,
)
...
def officeOfDeanRSPC(request):
    ...
    project = Project_Registration.objects.select_related(
        "PI_id__user", "PI_id__department"
    ).all()
    project1 = Project_Extension.objects.select_related(
        "project_id__PI_id__user", "project_id__PI_id__department"
    ).all()
    project2 = Project_Closure.objects.select_related(
        "project_id__PI_id__user", "project_id__PI_id__department"
    ).all()
    project3 = Project_Reallocation.objects.select_related(
        "project_id__PI_id__user", "project_id__PI_id__department"
    ).all()

    design = HoldsDesignation.objects.select_related("user", "designation").filter(
    ...
    return render(
        request, "officeModule/officeOfDeanRSPC/officeOfDeanRSPC.html", context
    )
...
def _req_history(req):
    ...
    return Tracking.objects.select_related(
        ...
        "receiver_id",
        "receive_design",
    ).filter(file_id=req.assign_file)
...
def officeOfDeanPnD(request):
    ...
    designations = [
        d.designation
        for d in HoldsDesignation.objects.select_related(
            "user", "designation", "working"
        ).filter(working=user)
    ]
    ...
    if "createassign" in request.POST:
        ...
        requisition = Requisitions.objects.select_related(
            "userid__user",
            "userid__department",
            "assign_file__uploader__user",
            "assign_file__uploader__department",
            "assign_file__designation",
        ).get(pk=req_id)
        ...
        for hold in holds:
            ...
            if str(hold.designation.name) == "Civil_JE":
                ...
                sender_design = hold
                receive = HoldsDesignation.objects.select_related(
                    "user", "designation", "working"
                ).get(designation__name="Civil_AE")
                # fdate = datetime.dat
                ...
                receive = HoldsDesignation.objects.select_related(
                    "user", "designation", "working"
                ).get(designation__name="Electrical_AE")
    ...
    # kept in the database for record-keeping reasons.
        ...
        hold = HoldsDesignation.objects.select_related(
            "user", "designation", "working"
        ).get(working=user, designation__name__in=_pnd_deslist)
        if hold:
            req_id = request.POST.get("req_id")
            try:
                req = Requisitions.objects.select_related(
                    "userid__user",
                    "userid__department",
                    "assign_file__uploader__user",
                    "assign_file__uploader__department",
    ...
    req = Requisitions.objects.select_related(
        ...
        "assign_file__uploader__department",
        "assign_file__designation",
    ).filter(assign_file__isnull=True, tag=0)
    # all requisitions
    all_req = Requisitions.objects.select_related(
        "userid__user",
        "userid__department",
        "assign_file__uploader__user",
        "assign_file__uploader__department",
    ...
    assigned_req = list(
        Requisitions.objects.select_related(
            "userid__user",
            "userid__department",
            "assign_file__uploader__user",
            "assign_file__uploader__department",
        ...
        .filter(assign_file__isnull=False)
        .select_related()
    )
    # use list comprehension to create a list of pairs of (tracking file, corresponding requisition)
    # for incoming tracking files
    incoming_files = [
        (f, _list_find(assigned_req, lambda r: r.assign_file == f.file_id))
        for f in Tracking.objects.select_related(
            "file_id__uploader__user",
            "file_id__uploader__department",
            "file_id__designation",
            "current_id__user",
    ...
    for r in all_req:
        ...
        if r.assign_file:
            ...
            passed = [r.assign_file.designation] + [
                t.receive_design for t in Tracking.objects.filter(file_id=r.assign_file)
            ]
...
def action(request):
    ...
    track = (
        ...
        .filter(file_id=requisition.assign_file)
        .filter(receiver_id=user)
        .get(is_read=False)
    )
    ...
    current_hold_design = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .filter(user=user)
        .get(designation=current_design)
    )
    ...
    if "Send" in request.POST:
        ...
        sent_hold_design = HoldsDesignation.objects.select_related(
            "user", "designation", "working"
        ).get(designation=sent_design)
        Tracking.objects.create(
            file_id=requisition.assign_file,
            current_id=extrainfo,
        ...
        office_dean_PnD_notif(
            request.user, sent_hold_design.working, "assignment_received"
        )
        """
        ...
        description = (
            description
            + " This assignment has been rejected. No further changes to this assignment are possible. Please create new requisition if needed."
        )
        Tracking.objects.create(
            file_id=requisition.assign_file,
            current_id=extrainfo,
        ...
        description = (
            description
            + " This assignment has been approved. No further changes to this assignment are possible. Please create new requisition if needed."
        )
        Tracking.objects.create(
            file_id=requisition.assign_file,
            current_id=extrainfo,
...
def frequest(request):
    ...
    return render(
        request,
        "officeModule/officeOfDeanPnD/viewRequisitions_content2.html",
        {"form": form},
    )
...
def officeOfPurchaseOfficr(request):
    ...
    """
    return render(
        request, "officeModule/officeOfPurchaseOfficer/officeOfPurchaseOfficer.html", {}
    )
...
def officeOfRegistrar(request):
    ...
    director_track = Tracking.objects.select_related(
        "file_id__uploader__user",
        "file_id__uploader__department",
        "file_id__designation",
        "current_id__user",
        ...
        receive_design__name="Registrar", current_design__designation__name="Director"
    )
    director_view = {}
    ...
    return render(
        request, "officeModule/officeOfRegistrar/officeOfRegistrar.html", context
    )
...
def officeOfRegistrar_forward(request, id):
    ...
    context = {"track_id": id}
    return render(
        request, "officeModule/officeOfRegistrar/forwardingForm.html", context
    )
...
def officeOfRegistrar_forward_submit(request):
    ...
    if request.method == "POST":
        try:
            ...
            t_id = Tracking.objects.select_related(
                "file_id__uploader__user",
                "file_id__uploader__department",
                "file_id__designation",
                "current_id__user",
            ...
            up_file = Tracking.objects.select_related(
                "file_id__uploader__user",
                "file_id__uploader__department",
                "file_id__designation",
                "current_id__user",
            ...
            rr = Registrar_response(
                track_id=Tracking.objects.select_related(
                    "file_id__uploader__user",
                    "file_id__uploader__department",
                    "file_id__designation",
                    "current_id__user",
...
def upload(request):
    ...
    print(file)
    request = registrar_create_doc(
        file_name=docname, purpose=purpose, Description=description, file=file
    )
    request.save()
    print(request)
...
def project_register(request):
    ...
    if len(sponsored_agency) is 0 and amount_sanctioned > "0":
        messages.error(
            request,
            "Error in Project Registration form: Amount cannot be sanctioned without Agency",
        )
        return HttpResponseRedirect("/profile/")
    ...
    if project_operated == "Only By PI" and len(CO_PI) > 0:
        messages.error(
            request, "Error in Project Registration form: Select PI and Co_PI in option"
        )
        return HttpResponseRedirect("/profile/")
    ...
    if len(sponsored_agency) is 0 and file:
        messages.error(
            request,
            "Error in Project Registration form: Enter agency name mentioned on MOU",
        )
        return HttpResponseRedirect("/profile/")
    ...
    if fund_recieved_date is not None and start_date < fund_recieved_date:
        messages.error(
            request,
            "Error in Project Registration form: Project cannot be started before receiving fund",
        )
        return HttpResponseRedirect("/profile/")
    ...
    if currentDate > start_date:
        messages.error(
            request,
            "Error in Project Registration form: You cannot start project without applying",
        )
        return HttpResponseRedirect("/profile/")

    """Save the Details to Project_Registration Table"""
    request_obj = Project_Registration(
        PI_id=extrainfo,
        ...
        duration=duration,
        fund_recieved_date=fund_recieved_date,
...
def project_registration_permission(request):
    ...
    if "approve" in request.POST or (
        request.method == "GET" and request.GET["a"] == "approve"
    ):
        ...
        for id in id_list:
            ...
            if "Pending" in obj.DRSPC_response or "Disapprove" in obj.DRSPC_response:
                ...
                if ptype == "sponsoered research":
                    emp_projects = emp_research_projects(
                        pi=pi,
                        co_pi=co_pi,
                        title=title,
                    ...
                    emp_projects = emp_consultancy_projects(
                        consultants=pi,
                        title=title,
                        client=funding_agency,
                        start_date=start_date,
                ...
                office_module_DeanRSPC_notif(
                    request.user, obj.PI_id.user, obj.DRSPC_response
                )
...
def project_extension(request):
    ...
    try:
        ob = Project_Registration.objects.select_related(
            "PI_id__user", "PI_id__department"
        ).get(pk=project_id)
    except:
    ...
    if extrainfo.id == ob.PI_id.id:
        ...
        if ob.DRSPC_response == "Approve":
            ...
            request_obj2 = Project_Extension(
                project_id=ob,
                date=date,
...
def project_closure(request):
    ...
    project_id = request.POST.get("project_id")
    try:
        extrainfo1 = Project_Registration.objects.select_related(
            "PI_id__user", "PI_id__department"
        ).get(pk=project_id)
    except:
        messages.error(request, "Project ID not found! Try again")
    ...
    if extrainfo.id == extrainfo1.PI_id.id:
        ...
        if extrainfo1.DRSPC_response == "Approve":
            request_obj1 = Project_Closure(
                project_id=extrainfo1,
                completion_date=completion_date,
                expenses_dues=expenses_dues,
                expenses_dues_description=expenses_dues_description,
                payment_dues=payment_dues,
                payment_dues_description=payment_dues_description,
                salary_dues=salary_dues,
                salary_dues_description=salary_dues_description,
                advances_dues=advances_dues,
                advances_description=advances_description,
                others_dues=others_dues,
                other_dues_description=other_dues_description,
...
def project_reallocation(request):
    ...
    project_id = request.POST.get("project_id")
    try:
        ob1 = Project_Registration.objects.select_related(
            "PI_id__user", "PI_id__department"
        ).get(pk=project_id)
    except:
        messages.error(request, "Project ID not found! Try again")
    ...
    if extrainfo.id == ob1.PI_id.id:
        ...
        if ob1.DRSPC_response == "Approve":
            request_obj3 = Project_Reallocation(
                project_id=ob1,
                date=applied_date,
                previous_budget_head=pbh,
                previous_amount=p_amount,
                new_budget_head=nbh,
                new_amount=n_amount,
...
def reg_details(request, pr_id):
    ...
    return render(
        request, "officeModule/officeOfDeanRSPC/view_details.html", {"obj": obj}
    )
...
def ext_details(request, pr_id):
    ...
    return render(
        request,
        "officeModule/officeOfDeanRSPC/extension_details.html",
        {"obj": obj, "pr": pr},
    )
...
def reallocate_details(request, pr_id):
    ...
    return render(
        request,
        "officeModule/officeOfDeanRSPC/reallocation_details.html",
        {"obj": obj, "pr": pr},
    )
...
def closure_details(request, pr_id):
    ...
    return render(
        request,
        "officeModule/officeOfDeanRSPC/closure_details.html",
        {"obj": obj, "pr": pr},
    )
...
def hod_action(request):
    ...
    if "forward" in request.POST:
        ...
        obj = Project_Registration.objects.select_related(
            "PI_id__user", "PI_id__department"
        ).get(pk=id)
...
def hod_extension(request):
    ...
    if "forward" in request.POST:
        id = request.POST.get("id")
        obj = Project_Extension.objects.select_related(
            "project_id__PI_id__user", "project_id__PI_id__department"
        ).get(pk=id)
        print(obj.HOD_response)
        if obj.HOD_response == "Pending" or obj.HOD_response == "pending":
...
def hod_allocation(request):
    ...
    if "forward" in request.POST:
        id = request.POST.get("id")
        obj = Project_Reallocation.objects.select_related(
            "project_id__PI_id__user", "project_id__PI_id__department"
        ).get(pk=id)
        print(obj.HOD_response)
        if obj.HOD_response == "Pending" or obj.HOD_response == "pending":
...
def hod_work(request):
    ...
    request_obj1 = Assigned_Teaching_credits(
        roll_no=tc, assigned_course=assigned_course
    )
    request_obj1.save()
    tc.tag = 1
    tc.save()
...
def newordershod(request):
    ...
    if request.method == "POST":
        ...
        try:
            obj = apply_for_purchase.objects.select_related(
                "indentor_name__user", "indentor_name__department"
            ).get(id=objid)
...
def newordersdirectorview(request):
    ...
    return render(
        request,
        "officeModule/officeOfPurchaseOfficer/viewordersdirector.html",
        context=context,
    )
...
def add_items(request):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "officeModule/officeOfPurchaseOfficer/manageStore_content2.html",
            {},
        )
    else:
        return render(
...
def after_purchase(request):
    ...
    if request.method == "POST":
        ...
        apply_for_purchase.objects.select_related(
            "indentor_name__user", "indentor_name__department"
        ).filter(id=file_no).update(amount=amount, invoice=invoice)

        return render(
            request, "officeModule/officeOfPurchaseOfficer/after_purchase.html", {}
        )
    else:
        return render(
            request, "officeModule/officeOfPurchaseOfficer/after_purchase.html", {}
...
def officeOfPurchaseOfficer(request):
    ...
    if (
        user_type == "DeputyRegistrar"
        or user_type == "Registrar"
        or user_type == "PurchaseOfficer"
    ):
        utype = 1
    ...
    if (
        per_user == "pkhanna"
        or user_type == "DeputyRegistrar"
        or user_type == "Registrar"
        or user_type == "HOD"
        or user_type == "PurchaseOfficer"
        or user_type == "Director"
        or user_type == "director"
    ):
        utype2 = 1
    ...
    if (
        user_type == "AssociateProfessor"
        or user_type == "AssistantProfessor"
        or user_type == "HOD"
        or user_type == "Director"
        or user_type == "director"
    ):  # give more such equivalent designations
        utype3 = 1
    ...
    if request.method == "POST":
        ...
        if "submit" in request.POST:
            ...
            return render(
                request,
                "officeModule/officeOfPurchaseOfficer/officeOfPurchaseOfficer.html",
                {},
            )
            ...
            if user_type == "HOD" or per_user == "pkhanna":
                ...
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvalHOD2.html",
                    context=context,
                )
                ...
                user_type == "DeputyRegistrar"
                or user_type == "Registrar"
                or per_user == "swapnali"
            ):
                alldata = (
                    apply_for_purchase.objects.select_related(
                        "indentor_name__user", "indentor_name__department"
                    )
                    .filter(HOD_approve_tag=1, registrar_approve_tag=0)
                    .order_by("-id")
                )
                ...
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvalRegistrar.html",
                    context=context,
                )
                ...
                alldata = (
                    ...
                    .filter(expected_cost__gte=50001, director_approve_tag=0)
                    .order_by("-id")
                )
                print(alldata)
                context = {"alldata": alldata, "des": user_type}
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvaldirector.html",
                    context=context,
                )
                ...
                alldata = (
                    ...
                    .filter(director_approve_tag=1, gem_tag=0)
                    .order_by("-id")
                )
                ...
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvalpurchaseofficer.html",
                    context=context,
                )
            ...
            if user_type == "HOD" or per_user == "pkhanna":
                ...
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvedHOD.html",
                    context=context,
                )
                ...
                alldata = (
                    ...
                    .filter(registrar_approve_tag=1, expected_cost__lte=50000)
                    .order_by("-id")
                )
                context = {"alldata": alldata, "des": user_type}
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvedRegistrar.html",
                    context=context,
                )
                ...
                alldata = (
                    ...
                    .filter(expected_cost__gte=50001, director_approve_tag=1)
                    .order_by("-id")
                )
                # print(alldata)
                context = {"alldata": alldata}
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/approvedDirector.html",
                    context=context,
                )
            ...
            if user_type == "PurchaseOfficer" or user_type == "purchseofficer":
                ...
                alldata = (
                    ...
                    .filter(director_approve_tag=1)
                    .exclude(gem_tag=0)
                )
                context = {"alldata": alldata, "des": user_type}
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/checkedpurchaseofficer.html",
                    context=context,
                )
            ...
            if user_type == "Registrar" or user_type == "DeputyRegistrar":
                ...
                return render(
                    request,
                    "officeModule/officeOfPurchaseOfficer/forwardedRegistrar.html",
                    context=context,
                )
            ...
            if item_type == 0:
                ...
                it = "non-consumable"
            str2 = (
                "item " + item_name + "of type " + it + " has been added to the store"
            )
            ...
            return render(
                request,
                "officeModule/officeOfPurchaseOfficer/manageStore_content1.html",
                {"q": q},
            )
            # return HttpResponse(1)
            ...
            alldata = apply_for_purchase.objects.select_related(
                "indentor_name__user", "indentor_name__department"
            ).filter(indentor_name=user_details)
            print(alldata)
            return render(
                request,
                "officeModule/officeOfPurchaseOfficer/purchaseHistory_content1.html",
                {"alldata": alldata},
            )
            ...
            return render(
                request,
                "officeModule/officeOfPurchaseOfficer/purchaseHistory_content2.html",
                {"alldata": alldata},
            )

        elif "purchase_search" in request.POST:
            pr = request.POST["file"]
            phmatch = apply_for_purchase.objects.select_related(
                "indentor_name__user", "indentor_name__department"
            ).filter(Q(id=pr))
            return render(
                request,
        ...
        q = stock.objects.all()
        ph = apply_for_purchase.objects.select_related(
            "indentor_name__user", "indentor_name__department"
        ).all()
...
def directorOffice(request):
    if request.user.is_authenticated:
        ...
        holds = HoldsDesignation.objects.select_related(
            "user", "designation", "working"
        ...
        if user.user_type == "faculty":
            return render(
                request, "officeModule/directorOffice/directorOffice.html", context
            )
        else:
            return render(
                request, "officeModule/directorOffice/unauthorised.html", context
            )
...
def viewProfile(request):
    ...
    cs = (
        Faculty.objects.select_related("id__user", "id__department")
        .all()
        .filter(id__department__name="CSE")
        .count()
    )
    ec = (
        Faculty.objects.select_related("id__user", "id__department")
        .all()
        .filter(id__department__name="ECE")
        .count()
    )
    me = (
        Faculty.objects.select_related("id__user", "id__department")
        .all()
        .filter(id__department__name="ME")
        .count()
    )
    des = (
        Faculty.objects.select_related("id__user", "id__department")
        .all()
        .filter(id__department__name="DESIGN")
        .count()
    )
    ns = (
        Faculty.objects.select_related("id__user", "id__department")
        .all()
        .filter(id__department__name="NATURAL SCIENCE")
        .count()
    )
    # Top students of each year
    top_2017_cse = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2017", id__department__name="CSE")
        .order_by("-cpi")[:3]
    )

    top_2016_cse = Student.objects.filter(
        id__id__startswith="2016", id__department__name="CSE"
    ).order_by("-cpi")[:3]

    top_2015_cse = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2015", id__department__name="CSE")
        .order_by("-cpi")[:3]
    )

    top_2017_me = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2017", id__department__name="ME")
        .order_by("-cpi")[:3]
    )

    top_2016_me = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2016", id__department__name="ME")
        .order_by("-cpi")[:3]
    )

    top_2015_me = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2015", id__department__name="ME")
        .order_by("-cpi")[:3]
    )

    top_2017_ece = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2017", id__department__name="ECE")
        .order_by("-cpi")[:3]
    )

    top_2016_ece = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2016", id__department__name="ECE")
        .order_by("-cpi")[:3]
    )

    top_2015_ece = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2015", id__department__name="ECE")
        .order_by("-cpi")[:3]
    )

    top_2017_design = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2017", id__department__name="DESIGN")
        .order_by("-cpi")[:3]
    )

    top_2016_design = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2016", id__department__name="DESIGN")
        .order_by("-cpi")[:3]
    )

    top_2015_design = (
        Student.objects.select_related("id__user", "id__department")
        .filter(id__id__startswith="2015", id__department__name="DESIGN")
        .order_by("-cpi")[:3]
    )
    ...
    context = {
        ...
        "top_17_cse": top_17_cse,
        "top_16_cse": top_16_cse,
        "top_15_cse": top_15_cse,
        "top_17_ece": top_17_ece,
        "top_16_ece": top_16_ece,
        "top_15_ece": top_15_ece,
        "top_17_me": top_17_me,
        "top_16_me": top_16_me,
        "top_15_me": top_15_me,
        "top_17_design": top_17_design,
        "top_16_design": top_16_design,
        "top_15_design": top_15_design,
    }
...
def viewOfficeBearers(request):
    ...
    for c in club_info:
        ...
        club_details.append(
            c.co_ordinator.id.user.first_name + " " + c.co_ordinator.id.user.last_name
        )
        club_details.append(
            c.co_coordinator.id.user.first_name
            + " "
            + c.co_coordinator.id.user.last_name
        )
        club_details.append(
            c.faculty_incharge.id.user.first_name
            + " "
            + c.faculty_incharge.id.user.last_name
        )
...
def viewFacProfile(request):
    ...
    context = {
        ...
        "des_faculty": des_faculty,
        "ns_faculty": ns_faculty,
    }
...
def viewStaffProfile(request):
    ...
    acad = (
        Staff.objects.select_related("id__user", "id__department")
        .all()
        .filter(
            Q(id__department__name="Academics")
            | Q(id__department__name="NATURAL SCIENCE")
            | Q(id__department__name="CSE")
            | Q(id__department__name="ECE")
            | Q(id__department__name="ME")
            | Q(id__department__name="DESIGN")
            | Q(id__department__name="MECHATRONICS")
            | Q(id__department__name="Workshop")
            | Q(id__department__name="Computer Centre")
        )
    )
    ...
    admin = (
        ...
        .all()
        .filter(
            Q(id__department__name="General Administration")
            | Q(id__department__name="Finance and Accounts")
            | Q(id__department__name="Purchase and Store")
            | Q(id__department__name="Registrar Office")
            | Q(id__department__name="Security and Central Mess")
        )
    )
    ...
    place = (
        Staff.objects.select_related("id__user", "id__department")
        .all()
        .filter(Q(id__department__name="Placement Cell"))
    )
    placement = []
    for x in place:
    ...
    offc = (
        ...
        .all()
        .filter(
            Q(id__department__name="Student Affairs")
            | Q(id__department__name="Office of The Dean P&D")
            | Q(id__department__name="Directorate")
            | Q(id__department__name="Office of The Dean R&D")
        )
    )
    office = []
    for x in offc:
    ...
    other = (
        ...
        .all()
        .filter(
            Q(id__department__name="Establishment & P&S")
            | Q(id__department__name="IWD")
            | Q(id__department__name="F&A & GA")
            | Q(id__department__name="Establishment, RTI and Rajbhasha")
            | Q(id__department__name="Establishment")
        )
    )
    others = []
    for x in other:
    ...
    if request.is_ajax():
        ...
        if programme in ("M.Tech", "M.Des", "PhD"):
            studentsearch = (
                Student.objects.select_related("id__user", "id__department")
                .all()
                .filter(id__id__startswith=yr, id__department__name=department)
            ...
            studentsearch = (
                ...
                .all()
                .filter(
                    id__id__startswith=year,
                    programme=programme,
                    id__department__name=department,
                )
            )
...
def meeting(request):
    ...
    if len(members) != len(fetched_members):
        ...
        Meeting.objects.create(
            agenda=agenda, time=meeting_time, date=adate, venue=venue
        )

        meeting_id = Meeting.objects.get(
            agenda=agenda, time=meeting_time, venue=venue, date=adate
        )
        for x in members:
            splitted_name = str(x).split(" ")
            u = User.objects.get(
                first_name=splitted_name[0], last_name=splitted_name[1]
            )
            e = ExtraInfo.objects.select_related("user", "department").get(user=u.id)
            f = Faculty.objects.select_related("id__user", "id__department").get(
                id=e.id
            )
            Member.objects.create(meeting_id=meeting_id, member_id=f)
...
def planMeetings(request):
    ...
    for x in meeting_info:
        ...
        for y in Members:
            members.append(
                y.member_id.id.user.first_name + " " + y.member_id.id.user.last_name
            )
...
def viewHOD(request):
    ...
    cse_hod = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .all()
        .filter(designation__name="CSE HOD")
    )
    ece_hod = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .all()
        .filter(designation__name="HOD (ECE)")
    )
    me_hod = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .all()
        .filter(designation__name="HOD (ME)")
    )
    ns_hod = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .all()
        .filter(designation__name="HOD (NS)")
    )
    des_hod = (
        HoldsDesignation.objects.select_related("user", "designation", "working")
        .all()
        .filter(designation__name="HOD (DESIGN)")
    )
...
def officeOfDeanAcademics(request):
    ...
    spi = Spi.objects.select_related(
        "student_id__id__user", "student_id__id__department"
    ).all()
    ...
    if user.user_type == "faculty":
        return render(
            request,
            "officeModule/officeOfDeanAcademics/officeOfDeanAcademics.html",
            context,
        )
    else:
