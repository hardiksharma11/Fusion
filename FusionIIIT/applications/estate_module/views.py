...
def estate(request):
    ...
    inventoryType_tabs = {"All": InventoryType.objects.all()}
...
def newWork(request):
    ...
    if new_work_form.is_valid():
        ...
        messages.success(
            request,
            f"New { new_work.get_workType_display() } Work Created: { new_work.name }",
        )
        return redirect("estate_module_home")
...
def editWork(request, work_id):
    ...
    if edit_work_form.is_valid():
        ...
        messages.success(
            request,
            f"Updated { edited_work.get_workType_display() } Work: { edited_work.name }",
        )
        return redirect("estate_module_home")
...
def newSubWork(request):
    ...
    if new_sub_work_form.is_valid():
        ...
        messages.success(request, f"New SubWork Created: { new_sub_work.name }")
        return redirect("estate_module_home")
...
def editInventoryType(request, inventoryType_id):
    ...
    edit_inventory_type_form = InventoryTypeForm(request.POST, instance=inventory_type)
...
def deleteInventoryType(request, inventoryType_id):
    ...
    messages.success(request, f"Inventory Type Deleted: { inventory_type_name }")
    return redirect("estate_module_home")
...
def editInventoryConsumable(request, inventory_consumable_id):
    ...
    inventory_consumable = InventoryConsumable.objects.get(pk=inventory_consumable_id)
...
def newInventoryNonConsumable(request):
    ...
    if new_inventory_non_consumable_form.is_valid():
        ...
        messages.success(
            request,
            f"New Non-Consumable Inventory Created: { new_inventory_non_consumable }",
        )
        return redirect("estate_module_home")
...
def editInventoryNonConsumable(request, inventory_non_consumable_id):
    ...
    if edit_inventory_non_consumable_form.is_valid():
        ...
        messages.success(
            request,
            f"Non-Consumable Inventory Updated: { edited_inventory_non_consumable }",
        )
        return redirect("estate_module_home")
