...
class CounsellingCellConstants:
    ...
    MEETING_STATUS = (("status_accepted", "Accepted"), ("status_pending", "Pending"))


class FacultyCounsellingTeam(models.Model):
    faculty = models.ForeignKey(Faculty, on_delete=models.CASCADE)
    faculty_position = models.CharField(
        max_length=50, choices=CounsellingCellConstants.FACULTY_POSTIONS
    )
...
class StudentCounsellingTeam(models.Model):
    ...
    student_position = models.CharField(
        max_length=50, choices=CounsellingCellConstants.STUDENT_POSTIONS
    )
...
class CounsellingIssue(models.Model):
    ...
    issue_category = models.ForeignKey(
        CounsellingIssueCategory, on_delete=models.CASCADE
    )
    ...
    issue_status = models.CharField(
        max_length=20,
        choices=CounsellingCellConstants.ISSUE_STATUS,
...
class CounsellingFAQ(models.Model):
    ...
    counselling_category = models.ForeignKey(
        CounsellingIssueCategory, on_delete=models.CASCADE
    )
...
class CounsellingMeeting(models.Model):
    ...
    meeting_time = models.CharField(
        max_length=20, choices=CounsellingCellConstants.TIME
    )
...
class CounsellingMinutes(models.Model):
    counselling_meeting = models.ForeignKey(
        CounsellingMeeting, on_delete=models.CASCADE
    )
...
class StudentMeetingRequest(models.Model):
    ...
    requested_student_invitee = models.ForeignKey(
        StudentCounsellingTeam, on_delete=models.CASCADE, null=True, blank=True
    )
    requested_faculty_invitee = models.ForeignKey(
        FacultyCounsellingTeam, on_delete=models.CASCADE, null=True, blank=True
    )
    requested_meeting_status = models.CharField(
        max_length=20,
        choices=CounsellingCellConstants.MEETING_STATUS,
