...
from .handlers import add_counselling_faq, add_student_counsellors
...
def counselling_cell(request):
    ...
    student_coordinators = StudentCounsellingTeam.objects.filter(
        student_position="student_coordinator"
    ).select_related()
    student_guide = StudentCounsellingTeam.objects.filter(
        student_position="student_guide"
    ).select_related()
    statuses = []

    issues = CounsellingIssue.objects.filter(
        issue_status="status_unresolved"
    ).select_related()
    ...
    if extra_info.user_type == "student":
        statuses = CounsellingIssue.objects.filter(
        ...
        if student:
            ...
            if student.student_position == "student_guide":
                ...
                if student in newlist:
                    ...
                    issues = CounsellingIssue.objects.filter(
                        issue_status="status_unresolved",
                        student__in=student_and_student_guide[student],
                    ).select_related()
                else:
    ...
    if request.method == "POST" and request.FILES:
        ...
        for i in range(2, sheet.max_row + 1):
            ...
            if sheet.cell(i, 1).value:
                ...
                checkForSG = StudentCounsellingTeam.objects.filter(
                    student_id=sg_roll_no, student_position="student_guide"
                )
                if len(checkForSG) == 0:
                    return JsonResponse(
                        {"status": 1, "message": "Student Guide Not Found"}
                    )
            studentToStudentGuide[sg_roll_no].append(student_roll_no)
        for sg, students in studentToStudentGuide.items():
            sg = StudentCounsellingTeam.objects.filter(
                student_id=sg, student_position="student_guide"
            ).first()
            for student in students:
                mappedStudent = StudentCounsellingInfo(
                    student_guide=sg,
                    student_id=Student(id=ExtraInfo(user=User(username=student))),
                )
                mappedStudent.save()
    return HttpResponseRedirect("/counselling/")
