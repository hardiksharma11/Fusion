...
ROOM_TYPE = (("SingleBed", "SingleBed"), ("DoubleBed", "DoubleBed"), ("VIP", "VIP"))
...
class BookingDetail(models.Model):
    ...
    def __str__(self):
        return "%s ----> %s - %s id is %s and category is %s" % (
            self.id,
            self.visitor,
            self.status,
            self.id,
            self.visitor_category,
        )
