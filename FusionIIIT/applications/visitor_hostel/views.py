...
def visitorhostel(request):
    ...
    if user_designation == "Intender":
        all_bookings = (
            BookingDetail.objects.select_related("intender", "caretaker")
            .all()
        ...
        complete_bookings = (
            BookingDetail.objects.select_related("intender", "caretaker")
            .filter(booking_to__lt=datetime.datetime.today(), intender=user)
            .order_by("booking_from")
        )
        canceled_bookings = (
            BookingDetail.objects.select_related("intender", "caretaker")
        ...
        active_bookings = (
            ...
            .select_related("intender", "caretaker")
            .order_by("booking_from")
        )
        cancel_booking_request = (
            BookingDetail.objects.select_related("intender", "caretaker")
        ...
        complete_bookings = (
            ...
            .filter(
                ...
                booking_to__lt=datetime.datetime.today(),
            )
            .select_related()
            .order_by("booking_from")
        )
        canceled_bookings = (
            BookingDetail.objects.filter(status="Canceled")
            .select_related("intender", "caretaker")
            .order_by("booking_from")
        )
        cancel_booking_requested = (
        ...
        if booking.status == "CheckedIn":
            ...
            for visitor in booking.visitor.all():
                meal = MealRecord.objects.select_related(
                    "booking__intender", "booking__caretaker", "visitor", "room"
...
def get_active_bookings(request):
    if request.method == "POST":
        ...
        return render(
            request, "vhModule/visitorhostel.html", {"active_bookings": active_bookings}
        )
    else:
        return HttpResponseRedirect("/visitorhostel/")
...
def get_inactive_bookings(request):
    if request.method == "POST":
        inactive_bookings = BookingDetail.objects.select_related(
            "intender", "caretaker"
...
def update_booking(request):
    if request.method == "POST":
        ...
        booking = BookingDetail.objects.select_related("intender", "caretaker").get(
            id=booking_id
        )
        ...
        for booking in c_bookings:
            ...
            temp2 = forwarded_booking_details(booking_from, booking_to)
            forwarded_rooms[booking.id] = temp2
        return render(request, "visitorhostel/", {"forwarded_rooms": forwarded_rooms})
...
def confirm_booking(request):
    if request.method == "POST":
        ...
        bd = BookingDetail.objects.select_related("intender", "caretaker").get(
            id=booking_id
        )
        bd.status = "Confirmed"
...
def cancel_booking(request):
    if request.method == "POST":
        ...
        visitors_hostel_notif(
            request.user, booking.intender, "booking_cancellation_request_accepted"
        )
        return HttpResponseRedirect("/visitorhostel/")
    else:
        return HttpResponseRedirect("/visitorhostel/")
...
def cancel_booking_request(request):
    if request.method == "POST":
        ...
        incharge_name = HoldsDesignation.objects.select_related(
            "user", "working", "designation"
        ).filter(designation__name="VhIncharge")[1]
    ...
    if user:
        if request.method == "POST":
            ...
            try:
                meal = MealRecord.objects.select_related(
                    "booking__intender", "booking__caretaker", "visitor", "room"
                ).get(visitor=visitor, booking=booking, meal_date=date_1)
            except:
                meal = False
...
def room_availabity(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "vhModule/room-availability.html",
...
def bill_between_dates(request):
    if request.method == "POST":
        ...
        return render(
            request,
            "vhModule/booking_bw_dates.html",
            {
...
def forwarded_booking_details(date1, date2):
    ...
    forwarded_bookings = BookingDetail.objects.select_related(
        "intender", "caretaker"
    ).filter(
        Q(booking_from__lte=date1, booking_to__gte=date1, status="Forward")
        | Q(booking_from__gte=date1, booking_to__lte=date2, status="Forward")
