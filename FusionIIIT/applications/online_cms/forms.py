...
class QuizForm(forms.Form):
    ...
    name = forms.CharField(
        label="Quiz Name",
        max_length=20,  # store quiz name
        widget=forms.TextInput(attrs={"placeholder": "Name"}),
    )
    startdate = forms.DateTimeField(
        label="Start Date",
        widget=forms.SelectDateWidget(attrs={"placeholder": "StartDate"}),
    )  # date of when the quiz is started
    starttime = forms.TimeField(
        label="Start Time", widget=forms.TimeInput(attrs={"placeholder": "StartTime"})
    )
    ...
    endtime = forms.TimeField(
        label="End Time", widget=forms.TimeInput(attrs={"placeholder": "EndTime"})
    )
    negative_marks = forms.FloatField(
        label="Penalty", required=True, max_value=0
    )  # it is the penalty and always take the negative value
...
class QuestionFormObjective(forms.Form):
    ...
    answer = forms.IntegerField(
        label="Answer",
        widget=forms.Textarea(attrs={"placeholder": "Enter in between 1 & 5"}),
    )
...
class MarksForm(forms.Form):
    ...
    exam_type = forms.CharField(
        label="Exam Name", widget=forms.Select(choices=EXAM_TYPES)
    )
