...
class Project(models.Model):
    ...
    project_status = models.CharField(
        max_length=20, choices=Constants.RESUME_TYPE, default="COMPLETED"
    )
...
class Coauthor(models.Model):
    ...
    def __str__(self):
        return "{} - {}".format(
            self.publication_id.publication_title, self.coauthor_name
        )
...
class Achievement(models.Model):
    ...
    achievement_type = models.CharField(
        max_length=20, choices=Constants.ACHIEVEMENT_TYPE, default="OTHER"
    )
...
class Extracurricular(models.Model):
    ...
    event_type = models.CharField(
        max_length=20, choices=Constants.EVENT_TYPE, default="OTHER"
    )
    description = models.TextField(max_length=1000, default="", null=True, blank=True)
...
class NotifyStudent(models.Model):
    placement_type = models.CharField(
        max_length=20, choices=Constants.PLACEMENT_TYPE, default="PLACEMENT"
    )
...
class PlacementStatus(models.Model):
    ...
    invitation = models.CharField(
        max_length=20, choices=Constants.INVITATION_TYPE, default="PENDING"
    )
    placed = models.CharField(
        max_length=20, choices=Constants.PLACED_TYPE, default="NOT PLACED"
    )
    timestamp = models.DateTimeField(auto_now=True)
...
class StudentPlacement(models.Model):
    ...
    placement_date = models.DateField(
        _("Date"), default=datetime.date.today, null=True, blank=True
    )
