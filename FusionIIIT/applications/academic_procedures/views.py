...
def academic_procedures(request):
    ...
    user_details = ExtraInfo.objects.select_related("user", "department").get(
        user=request.user
    )
    ...
    if str(des.designation) == "student":
        obj = Student.objects.select_related("id", "id__user", "id__department").get(
            id=user_details.id
        )
        return HttpResponseRedirect("/academic-procedures/stu/")
        # return HttpResponseRedirect('/logout/')
    elif (
        str(des.designation) == "Associate Professor"
        or str(des.designation) == "Professor"
        or str(des.designation) == "Assistant Professor"
    ):
...
def academic_procedures_faculty(request):
    ...
    if str(des.designation) == "student":
        ...
        object_faculty = Faculty.objects.select_related(
        ...
        thesis_supervision_request_list = (
            ThesisTopicProcess.objects.all()
            .select_related()
            .filter(supervisor_id=object_faculty)
        )
        approved_thesis_request_list = thesis_supervision_request_list.filter(
            approval_supervisor=True
        )
        pending_thesis_request_list = thesis_supervision_request_list.filter(
            pending_supervisor=True
        )
        faculty_list = get_faculty_list()
        assistantship_request_list = AssistantshipClaim.objects.all()
        hod_assistantship_request_list = (
            assistantship_request_list.filter(ta_supervisor_remark=True)
            .filter(thesis_supervisor_remark=True)
            .filter(hod_approval=False)
        )
        hod_approved_assistantship = (
            assistantship_request_list.filter(ta_supervisor_remark=True)
            .filter(thesis_supervisor_remark=True)
            .filter(acad_approval=False)
        )
        ta_approved_assistantship_request_list = (
            AssistantshipClaim.objects.all().filter(ta_supervisor_remark=True)
        )
        thesis_approved_assistantship_request_list = (
            AssistantshipClaim.objects.all().filter(thesis_supervisor_remark=True)
        )
        approved_assistantship_request_list = (
            ta_approved_assistantship_request_list
            | thesis_approved_assistantship_request_list
        )
        mtechseminar_request_list = MTechGraduateSeminarReport.objects.all().filter(
            Overall_grade=""
        )
        phdprogress_request_list = PhDProgressExamination.objects.all().filter(
            Overall_grade=""
        )
        courses_list = list(
            CourseInstructor.objects.select_related(
                "course_id", "batch_id", "batch_id__discipline"
            )
            .filter(instructor_id__id=fac_id.id)
...
@login_required(login_url="/accounts/login")
def account(request):
    assistant_account_list = AssistantshipClaim.objects.filter(
        ta_supervisor_remark=True
    ).filter(thesis_supervisor_remark=True)
    assistant_pen_list = (
        AssistantshipClaim.objects.filter(ta_supervisor_remark=True)
        .filter(thesis_supervisor_remark=True)
        .filter(acad_approval=True)
        .filter(account_approval=False)
    )
    assistant_account_length = len(
        assistant_account_list.filter(acad_approval=True).filter(account_approval=False)
    )
    return render(
        request,
...
def academic_procedures_student(request):
    ...
    if str(des.designation) == "student":
        ...
        curr_sem_id = Semester.objects.get(
            curriculum=curr_id, semester_no=obj.curr_semester_no
        )

        try:
            semester_no = obj.curr_semester_no + 1
            next_sem_id = Semester.objects.get(
                curriculum=curr_id, semester_no=semester_no
            )
        ...
        student_registration_check_final = get_student_registrtion_check(
            obj, next_sem_id
        )
        ...
        pre_registration_date_flag, prd_start_date = get_pre_registration_eligibility(
            current_date, user_sem, year
        )
        final_registration_date_flag = get_final_registration_eligibility(current_date)
        add_or_drop_course_date_flag = get_add_or_drop_course_date_eligibility(
            current_date
        )
        ...
        if student_registration_check_final:
            final_registration_flag = (
                student_registration_check_final.final_registration_flag
            )

        # print(">>>>>>>>>>>>>>>>>>>>>>",student_registration_check_pre.pre_registration_flag)

        acad_year = get_acad_year(user_sem, year)
        currently_registered_courses = get_currently_registered_courses(
        ...
        final_registration_choice, unavailable_courses_nextsem = (
            get_final_registration_choices(next_sem_registration_courses, batch.year)
        )
        currently_registered_course = get_currently_registered_course(
            obj, obj.curr_semester_no
        )
        ...
        try:
            pre_registered_courses = InitialRegistration.objects.all().filter(
            ...
            for pre_registered_course in pre_registered_courses:
                ...
                if (
                    pre_registered_course.course_slot_id.name
                    not in pre_registered_course_show
                ):
                    pre_registered_course_show[
                        pre_registered_course.course_slot_id.name
                    ] = [
                        {
                            "course_code": pre_registered_course.course_id.code,
                            "course_name": pre_registered_course.course_id.name,
                            "course_credit": pre_registered_course.course_id.credit,
                            "priority": pre_registered_course.priority,
                        }
                    ]
                else:
                    pre_registered_course_show[
        ...
        try:
            final_registered_courses = FinalRegistration.objects.all().filter(
                student_id=user_details.id, semester_id=next_sem_id
            )
            final_registered_course_show = []
            for final_registered_course in final_registered_courses:
                final_registered_course_show.append(
                    {
                        "course_code": final_registered_course.course_id.code,
                        "course_name": final_registered_course.course_id.name,
                        "course_credit": final_registered_course.course_id.credit,
                    }
                )
            add_courses_options = get_add_course_options(
                current_sem_branch_course, currently_registered_course, batch.year
            )
        ...
        for i in currently_registered_courses:
            try:
                performance_obj = (
                    ...
                    .filter(student_id=obj, curr_id=i)
                    .first()
                )
            except Exception as e:
        ...
        for i in currently_registered_courses:
            try:
                result_announced_obj = MarkSubmissionCheck.objects.select_related().get(
                    curr_id=i
                )
        ...
        if masters_flag:
            faculty_list = get_faculty_list()
            thesis_request_list = ThesisTopicProcess.objects.all().filter(
                student_id=obj
            )
        ...
        if phd_flag:
            ...
            teaching_credit_registration_course = (
                Curriculum.objects.all().select_related().filter(batch=2016, sem=6)
            )
        ...
        if student_flag:
            try:
                ...
                obj = Dues.objects.select_related().get(
                    student_id=Student.objects.select_related(
                        "id", "id__user", "id__department"
                    ).get(id=request.user.username)
                )
        ...
        for i in course_list:
            ...
            instructors = Curriculum_Instructor.objects.select_related(
                "curriculum_id",
                "instructor_id",
                "curriculum_id__course_id",
                "instructor_id__department",
                "instructor_id__user",
            ).filter(curriculum_id=i)
            pr, ab = 0, 0
            for j in list(instructors):

                presents = Student_attendance.objects.select_related(
                    "student_id",
                    "student_id__id",
                    "student_id__id__user",
                    "student_id__id__department",
                    "instructor_id",
                    "instructor_id__curriculum_id",
                    "instructor_id__curriculum_id__course_id",
                    "instructor_id__instructor_id",
                    "instructor_id__instructor_id__user",
                    "instructor_id__instructor_id__department",
                ).filter(student_id=obj, instructor_id=j, present=True)

                absents = Student_attendance.objects.select_related(
                    "student_id",
                    "student_id__id",
                    "student_id__id__user",
                    "student_id__id__department",
                    ...
                    "instructor_id__instructor_id__user",
                    "instructor_id__instructor_id__department",
                ).filter(student_id=obj, instructor_id=j, present=False)
                pr += len(presents)
                ab += len(absents)
            attendence.append((i, pr, pr + ab))
        cur_spi = (
            "Sem results not available"  # To be fetched from db if result uploaded
        )
        ...
        return render(
            request,
            "../templates/academic_procedures/academic.html",
            {
                "details": details,
                # 'calendar': calendar,
...
def apply_branch_change(request):
    ...
    cpi_data = (
        Student.objects.all()
        .select_related("id", "id__user", "id__department")
        .filter(id=extraInfo_user.id)
        .first()
    )
...
def branch_change_request(request):
    ...
    if request.method == "POST":
        ...
        department = (
            DepartmentInfo.objects.all()
            .filter(id=int(request.POST["branches"]))
            .first()
        )
        change_save = BranchChange(branches=department, user=student)
        change_save.save()
        return HttpResponseRedirect("/academic-procedures/main")
    else:
        return HttpResponseRedirect("/academic-procedures/main")
...
def approve_branch_change(request):
    ...
    if request.method == "POST":
        ...
        for i in range(len(branches)):
            ...
            change = (
                BranchChange.objects.select_related(
                    "branches",
                    "user",
                    "user__id",
                    "user__id__user",
                    "user__id__department",
                )
                .all()
                .filter(user=student)
            )
        ...
        try:
            ...
            messages.info(
                request, "Unable to proceed, we will get back to you very soon"
            )
        return HttpResponseRedirect("/academic-procedures/main")
...
def dropcourseadmin(request):
    ...
    try:
        Register.objects.filter(
            curr_id=curriculum_object.first(), student_id=int(data[0])
        ).delete()
    except:
        pass
        # print("hello ")
...
def gen_course_list(request):
    if request.POST:
        try:
            ...
            obj = (
                course_registration.objects.filter(
                    course_id__id=course_id, student_id__batch=batch
                )
                .select_related("student_id__id__user", "student_id__id__department")
                .only(
                    "student_id__batch",
                    "student_id__id__user__first_name",
                    "student_id__id__user__last_name",
                    "student_id__id__department__name",
        ...
        for i in obj:
            students.append(
                {
                    "rollno": i.student_id.id.user.username,
                    "name": i.student_id.id.user.first_name
                    + " "
        ...
        html = render_to_string(
            "academic_procedures/gen_course_list.html",
...
def verify_course(request):
    ...
    if request.POST:
        ...
        html = render_to_string(
            "academic_procedures/studentCourses.html",
            {
                "details": details,
                "dict2": dict2,
...
def acad_branch_change(request):
    ...
    change_queries = BranchChange.objects.select_related(
        "branches", "user", "user__id", "user__id__user", "user__id__department"
    ).all()
    ...
    if len(initial_branch) > 0:
        tag = True
    context = {"list": lists, "total": len(initial_branch), "tag": tag}
    return render(
        request,
...
def get_pre_registration_eligibility(current_date, user_sem, year):
    ...
    try:
        ...
        pre_registration_date = (
            ...
            .filter(description=f"Pre Registration {user_sem} {year}")
            .first()
        )
...
def get_final_registration_eligibility(current_date):
    try:
        frd = (
            Calendar.objects.all()
            .filter(description="Physical Reporting at the Institute")
            .first()
        )
...
def get_add_or_drop_course_date_eligibility(current_date):
    try:
        add_drop_course_date = (
            Calendar.objects.all()
            .filter(description="Last Date for Adding/Dropping of course")
            .first()
        )
...
def get_course_verification_date_eligibilty(current_date):
    try:
        course_verification_date = (
            Calendar.objects.all()
            .filter(description="course verification date")
            .first()
        )
...
def pre_registration(request):
    if request.method == "POST":
        try:
            ...
            try:
                student_registeration_check = get_student_registrtion_check(
                    current_user, sem_id
                )
                if student_registeration_check.pre_registration_flag == True:
                    messages.error(
                        request, "You have already registered for next semester"
                    )
                    return HttpResponseRedirect("/academic-procedures/main")
            except Exception as e:
                # print(e)
            ...
            for course_slot in course_slots:
                course_priorities = request.POST.getlist(
                    "course_priority-" + course_slot
                )
                ...
                for course_priority in course_priorities:
                    priority_of_current_course, course_id = map(
                        int, course_priority.split("-")
                    )
...
def get_student_registrtion_check(obj, sem):
    return (
        StudentRegistrationChecks.objects.all()
        .filter(student_id=obj, semester_id=sem)
        .first()
    )
            ...
            try:
                ...
                try:
                    StudentRegistrationChecks.objects.filter(
                        student_id=current_user, semester_id=sem_id
...
def allot_courses(request):
    ...
    try:
        if request.method == "POST" and request.FILES:
            ...
            sem_id = Semester.objects.get(
                curriculum=batch.curriculum, semester_no=sem_no
            )
            ...
            for i in range(1, sheet.nrows):
                ...
                course_slot = CourseSlot.objects.get(
                    name=course_slot_name.strip(), semester=sem_id
                )
                course = Courses.objects.get(
                    code=course_code.strip(), name=course_name.strip()
                )
                # print(">>>>>",roll_no,course_slot_name,course_code,course_name)
                final_registration = FinalRegistration(
                    student_id=student,
                    course_slot_id=course_slot,
                    course_id=course,
                    semester_id=sem_id,
                )
        ...
        messages.error(
            request,
            "Error: Query does not match. Please check if all the data input is in the correct format.",
        )
        return HttpResponseRedirect("/academic-procedures/main")
        ...
        try:
            ...
            for i in range(1, count + 1):
                ...
                try:
                    ...
                    if course_registration.objects.filter(
                        student_id__batch_id__year=current_user.batch_id.year,
                        course_id=course_id,
                    ).count() < courseslot_id.max_registration_limit and (
                        course_registration.objects.filter(
                            course_id=course_id, student_id=current_user
                        ).count()
                        == 0
                    ):
    ...
    if request.method == "POST":
        try:
            if str(request.POST.get("by")) == "st":
                ...
                supervisor_faculty = get_object_or_404(
                    User, username=request.POST.get("supervisor")
                )
                supervisor_faculty = ExtraInfo.objects.select_related(
                    "user", "department"
                ).get(user=supervisor_faculty)
                supervisor_faculty = Faculty.objects.select_related(
                    "id", "id__user", "id__department"
                ).get(id=supervisor_faculty)

                try:
                    co_supervisor_faculty = get_object_or_404(
                        User, username=request.POST.get("co_supervisor")
                    )
                    co_supervisor_faculty = ExtraInfo.objects.select_related(
                        "user", "department"
                    ).get(user=co_supervisor_faculty)
                    co_supervisor_faculty = Faculty.objects.select_related(
                        "id", "id__user", "id__department"
                    ).get(id=co_supervisor_faculty)
                except Exception as e:
                ...
                try:
                    ...
                    curr_id = Curriculum.objects.select_related().get(
                ...
                member1 = ExtraInfo.objects.select_related("user", "department").get(
                    user=member1
                )
                member1 = Faculty.objects.select_related(
                    "id", "id__user", "id__department"
                ).get(id=member1)

                member2 = get_object_or_404(User, username=request.POST.get("member2"))
                member2 = ExtraInfo.objects.select_related("user", "department").get(
                    user=member2
                )
                member2 = Faculty.objects.select_related(
                    "id", "id__user", "id__department"
                ).get(id=member2)

                try:
                    member3 = get_object_or_404(
                        User, username=request.POST.get("member3")
                    )
                    member3 = ExtraInfo.objects.select_related(
                        "user", "department"
                    ).get(user=member3)
                    member3 = Faculty.objects.select_related(
                        "id", "id__user", "id__department"
                    ).get(id=member3)
                except Exception as e:
                    member3 = None
        ...
        for course in courseslot.courses.all():
            if (
                FinalRegistration.objects.filter(
                    student_id__batch_id__year=batch, course_id=course
                ).count()
                < max_limit
            ):
                lis.append(course)
...
def get_currently_registered_courses(id, user_sem):
    obj = (
        ...
        .filter(student_id=id, semester=user_sem)
    )
    ans = []
    for i in obj:
        course = Curriculum.objects.select_related().get(
            curriculum_id=i.curr_id.curriculum_id
        )
        ans.append(course)
    return ans
...
def acad_person(request):
    ...
    if str(des.designation) == "student":
        ...
        course_verification_date = get_course_verification_date_eligibilty(
            demo_date.date()
        )
        ...
        obj_list = (
            MarkSubmissionCheck.objects.all()
            .select_related()
            .filter(verified=False, submitted=True)
        )
        for i in obj_list:
...
def get_batch_grade_verification_data(list):
    ...
    batch_grade_data_set = {
        "batch_grade_data": [batch_1_list, batch_2_list, batch_3_list, batch_4_list],
...
def student_list(request):
    if request.POST and (request.GET["excel_export"] == "false"):
        ...
        student_obj = (
            FeePayments.objects.all()
            .select_related("student_id")
            .filter(student_id__batch_id=batch_id)
        )
        if student_obj:
            reg_table = (
                student_obj.prefetch_related("student_id__studentregistrationchecks")
                .filter(
                    semester_id=student_obj[0].semester_id,
                    student_id__studentregistrationchecks__final_registration_flag=True,
                )
                .select_related(
                    "student_id",
                    "student_id__id",
                ...
                .values(
                    ...
                    "student_id__batch",
                    "student_id__id__department__name",
                    "student_id__programme",
                    "student_id__curr_semester_no",
                    "student_id__id__sex",
                    "student_id__id__phone_no",
                    "student_id__category",
                    "student_id__specialization",
                    "mode",
                    "transaction_id",
                    "deposit_date",
                    "fee_paid",
                ...
                .order_by("student_id__id__user")
            )
        else:
            reg_table = []

        html = render_to_string(
            "academic_procedures/student_table.html", {"student": reg_table}, request
        )
        ...
        table = [
            (
                ...
                "Date for Fee deposition",
                "Mode",
                "If Loan account enter UTR Number",
                "Upload Fee Receipt path",
            )
        ]
        ...
        if student_obj:
            ...
            table += (
                ...
                .annotate(
                    ...
                    pwd_status=(
                        Value("YES", output_field=CharField())
                        if F("student_id__pwd_status") == True
                        else Value("NO", output_field=CharField())
                    ),
                    phone_no=F("student_id__id__phone_no"),
                    date_deposited=Concat(
                        Cast(ExtractDay("deposit_date"), CharField()),
                        Value("/"),
                        Cast(ExtractMonth("deposit_date"), CharField()),
                        Value("/"),
                        Cast(ExtractYear("deposit_date"), CharField()),
                        output_field=CharField(),
                    ),
                )
                .order_by("student_id__id__user__username")
                .values_list(
                    "admission_year",
                    "semester",
                    "roll_no",
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{batch_id.name}_{batch_id.discipline.acronym}_{batch_id.year}_final_registered.xlsx"'
            )
            return response
        else:
            return HttpResponse(
                json.dumps({"error": "No registered students found"}),
...
def course_list(request):
    if request.method == "POST":
        ...
        final_registration_table = FinalRegistration.objects.all().filter(
            semester_id=semester_id, verified=False
        )
        final = final_registration_table.filter(
            student_id=student_id, semester_id=semester_id
        )
        html = render_to_string(
            "academic_procedures/student_course_list.html",
            {"course_list": final},
            request,
        )
...
def verify_registration(request):
    ...
    if request.POST.get("status_req") == "accept":
        ...
        curr_id = batch.curriculum
        sem_id = Semester.objects.get(
            curriculum=curr_id, semester_no=student.curr_semester_no + 1
        )

        final_register_list = FinalRegistration.objects.all().filter(
            student_id=student, verified=False, semester_id=sem_id
        )
        ...
        with transaction.atomic():
            ...
            academics_module_notif(
                request.user, student.id.user, "registration_approved"
            )
        ...
        curr_id = batch.curriculum
        sem_id = Semester.objects.get(
            curriculum=curr_id, semester_no=student_id.curr_semester_no + 1
        )

        with transaction.atomic():
            academicadmin = get_object_or_404(User, username="acadadmin")
            FinalRegistration.objects.filter(
                student_id=student_id, verified=False, semester_id=sem_id
            ).delete()
            StudentRegistrationChecks.objects.filter(
                student_id=student_id, semester_id=sem_id
            ).update(final_registration_flag=False)
            ...
            academics_module_notif(
                academicadmin,
                student_id.id.user,
                "Registration Declined - " + reject_reason,
            )
            return JsonResponse(
    ...
    if request.method == "POST":
        try:
            ...
            student_id = (
                ExtraInfo.objects.all()
                .select_related("user", "department")
                .filter(user=student_id)
                .first()
            )
            student_id = (
                Student.objects.all()
                .select_related("id", "id__user", "id__department")
                .filter(id=student_id.id)
                .first()
            )

            course1 = Curriculum.objects.select_related().get(
                curriculum_id=request.POST.get("course1")
            )
            course2 = Curriculum.objects.select_related().get(
                curriculum_id=request.POST.get("course2")
            )
            course3 = Curriculum.objects.select_related().get(
                curriculum_id=request.POST.get("course3")
            )
            course4 = Curriculum.objects.select_related().get(
                curriculum_id=request.POST.get("course4")
            )
...
def course_marks_data(request):
    try:
        ...
        print(course_id)
        student_list = (
            course_registration.objects.filter(course_id__id=course_id)
            .select_related("student_id__id__user", "student_id__id__department")
            .only(
                "student_id__batch",
                "student_id__id__user__first_name",
        ...
        data = render_to_string(
            "academic_procedures/course_marks_data.html",
            {
                "enrolled_student_list": enrolled_student_list,
                "course": course,
                "grade_submission_date_eligibility": grade_submission_date_eligibility,
            },
            request,
        )
...
def submit_marks(request):
    try:
        ...
        for x in range(values_length):
            ...
            student_id = ExtraInfo.objects.select_related("user", "department").get(
                id=student_id
            )
            student_id = Student.objects.select_related(
                "id", "id__user", "id__department"
            ...
            if request.POST.get("final_submit") == "True":
                try:
                    o_sub = MarkSubmissionCheck.objects.select_related().get(
                        curr_id=curr_id
                    )
                except Exception as e:
                    o_sub = None
            ...
            if request.POST.get("final_submit") == "False":
                try:
                    sub_obj = MarkSubmissionCheck.objects.select_related().get(
                        curr_id=curr_id
                    )
                except Exception as e:
                    sub_obj = None
...
def verify_course_marks_data(request):
    try:
        ...
        data = render_to_string(
            "academic_procedures/verify_course_marks_data.html",
            {
                "enrolled_student_list": enrolled_student_list,
                "course": course,
                "grade_verification_date_eligibility": grade_verification_date_eligibility,
            },
            request,
        )
        obj = json.dumps({"data": data})
...
def generate_grade_pdf(request):
    instructor = (
        Curriculum_Instructor.objects.all()
        .select_related(
            "curriculum_id",
            "instructor_id",
            "curriculum_id__course_id",
        ...
        .filter(curriculum_id=verified_marks_students_curr)
        .first()
    )
    context = {
        "verified_marks_students": verified_marks_students,
    ...
    if pdf:
        ...
        response["Content-Disposition"] = 'attachment; filename="%s.pdf"' % (
            verified_marks_students_curr.course_code
        )
        return response
    return HttpResponse("PDF could not be generated")
...
def generate_result_pdf(request):
    ...
    curriculum_obj = (
        Curriculum.objects.all()
        .select_related()
        .filter(batch=int(batch))
        .filter(branch=str(branch))
        .filter(programme=programme)
    )
    curriculum_obj_common = (
        Curriculum.objects.all()
        .select_related()
        .filter(batch=int(batch))
...
def get_spi(course_list, grade_list):
    ...
    for i in range(2, len(grade_list)):
        x = {"grade": grade_list[i], "credits": None}
        y.append(x)
...
def manual_grade_submission(request):
    if request.method == "POST" and request.FILES:
        ...
        if not curriculum_obj:
            ...
            if not course_obj:
                course_obj_create = Course(
                    course_name=course_name, course_details=instructor
                )
        ...
        marks_check_obj = (
            MarkSubmissionCheck.objects.select_related()
            .all()
            .filter(curr_id=curriculum_obj)
            .first()
        )
        if marks_check_obj:
            ...
            marks_check_obj_create = MarkSubmissionCheck(
                curr_id=curriculum_obj, submitted=True, verified=False, announced=False
            )
        ...
        for i in range(11, sheet.nrows):
            ...
            extrainfo = ExtraInfo.objects.select_related("user", "department").get(
                user=user
            )
            dep_objects = DepartmentInfo.objects.get(name=str(branch))
            ...
            register_obj = (
                Register.objects.all()
                .filter(curr_id=curriculum_obj, student_id=student_obj)
                .first()
            )
            if not register_obj:
...
def test(request):
    ...
    for i in st_list:
        ...
        if i.programme.upper() == "B.DES" or i.programme.upper() == "B.TECH":
            ...
            i.programme.upper() == "M.DES"
            or i.programme.upper() == "M.TECH"
            or i.programme.upper() == "PH.D"
        ):
            batch = int("20" + roll[:2])
            i.batch = batch
        br_up.append(i)
...
def test_ret(request):
    try:
        data = render_to_string("academic_procedures/test_render.html", {}, request)
...
def Bonafide_form(request):
    ...
    name = (
        ExtraInfo.objects.all()
        .select_related("user", "department")
        .filter(id=request.user.username)[0]
        .user
    )
...
def ACF(request):
    ...
    acf = AssistantshipClaim(
        student=stu,
        month=month,
        year=year,
        bank_account=account,
        thesis_supervisor=faculty_inc2,
        ta_supervisor=faculty_inc1,
        applicability=appli,
    )
    acf.save()
...
def update_assistantship(request):
    if request.method == "POST":
        ...
        if (
            assistantship_object.thesis_supervisor_remark == True
            and assistantship_object.ta_supervisor_remark == True
        ):
...
def MTSGF(request):
    if request.method == "POST":
        ...
        form = MTechGraduateSeminarReport(
            student=stu,
            theme_of_work=theme,
            date=date,
            place=place,
            time=time,
            work_done_till_previous_sem=work,
            specific_contri_in_cur_sem=contribution,
            future_plan=future,
            brief_report=report,
            publication_submitted=publication_submitted,
            publication_accepted=publication_accepted,
            paper_presented=paper_presented,
            papers_under_review=paper_under_review,
        )
        form.save()
...
def PHDPE(request):
    if request.method == "POST":
        ...
        form = PhDProgressExamination(
            student=stu,
            theme=theme,
            seminar_date_time=dateandtime,
            place=place,
            work_done=work,
            specific_contri_curr_semester=contribution,
            future_plan=future,
            details=uploadfile,
            papers_published=paper_published,
            presented_papers=paper_presented,
            papers_submitted=paper_submitted,
        )
        form.save()
        message = "Form submitted succesfully"
        res = "success"
...
def update_dues(request):
    if request.method == "POST":
        ...
        if pd < 0 and -1 * pd > dues_object.placement_cell_due:
            message = (
                message + "Subtracting more value than existing placement cell due<br>"
            )
...
def mdue(request):
    if request.method == "POST":
        ...
        if messdue_list:
            new_remaining = (
                messdue_list[len(messdue_list) - 1].remaining_amount + amount1
            )
