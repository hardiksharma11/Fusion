...
def academic_procedures_faculty(request):
    ...
    if str(des.designation) == "student":
        ...
        return Response(
            {"error": "User is acadadmin"}, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        thesis_supervision_request_list = (
            faculty_object.thesistopicprocess_supervisor.all()
        )
        thesis_supervision_request_list_data = serializers.ThesisTopicProcessSerializer(
            thesis_supervision_request_list, many=True
        ).data
        approved_thesis_request_list = serializers.ThesisTopicProcessSerializer(
            thesis_supervision_request_list.filter(approval_supervisor=True), many=True
        ).data
        pending_thesis_request_list = serializers.ThesisTopicProcessSerializer(
            thesis_supervision_request_list.filter(pending_supervisor=True), many=True
        ).data
        courses_list = serializers.CurriculumInstructorSerializer(
            user_details.curriculum_instructor_set.all(), many=True
        ).data
...
def academic_procedures_student(request):
    ...
    if str(des.designation) == "student":
        ...
        if obj.programme.upper() == "PH.D":
            ...
            return Response(
                {"message": "Student has no record"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        currently_registered_courses_data = serializers.CurriculumSerializer(
            currently_registered_courses, many=True
        ).data
        try:
            pre_registered_courses = obj.initialregistrations_set.all().filter(
                semester=user_sem
            )
            pre_registered_courses_show = obj.initialregistrations_set.all().filter(
                semester=user_sem + 1
            )
        except:
        ...
        try:
            final_registered_courses = obj.finalregistrations_set.all().filter(
                semester=user_sem
            )
        except:
            final_registered_courses = None

        pre_registered_courses_data = serializers.InitialRegistrationsSerializer(
            pre_registered_courses, many=True
        ).data
        pre_registered_courses_show_data = serializers.InitialRegistrationsSerializer(
            pre_registered_courses_show, many=True
        ).data
        final_registered_courses_data = serializers.FinalRegistrationsSerializer(
            final_registered_courses, many=True
        ).data

        current_credits = get_current_credits(currently_registered_courses)

        next_sem_branch_courses = get_branch_courses(
            current_user, user_sem + 1, user_branch
        )
        next_sem_branch_courses_data = serializers.CurriculumSerializer(
            next_sem_branch_courses, many=True
        ).data

        fee_payment_mode_list = dict(Constants.PaymentMode)

        next_sem_branch_registration_courses = get_registration_courses(
            next_sem_branch_courses
        )
        next_sem_branch_registration_courses_data = []
        for choices in next_sem_branch_registration_courses:
            next_sem_branch_registration_courses_data.append(
                serializers.CurriculumSerializer(choices, many=True).data
            )
        ...
        final_registration_choices = get_registration_courses(
            get_branch_courses(request.user, user_sem, user_branch)
        )
        final_registration_choices_data = []
        for choices in final_registration_choices:
            final_registration_choices_data.append(
                serializers.CurriculumSerializer(choices, many=True).data
            )
        ...
        performance_list_data = serializers.SemesterMarksSerializer(
            performance_list, many=True
        ).data

        thesis_request_list = serializers.ThesisTopicProcessSerializer(
            obj.thesistopicprocess_set.all(), many=True
        ).data

        pre_existing_thesis_flag = True if obj.thesistopicprocess_set.all() else False

        current_sem_branch_courses = get_branch_courses(
            current_user, user_sem, user_branch
        )
        ...
        student_registration_check_pre = obj.studentregistrationcheck_set.all().filter(
            semester=user_sem + 1
        )
        student_registration_check_final = (
            obj.studentregistrationcheck_set.all().filter(semester=user_sem)
        )
        ...
        if phd_flag:
            teaching_credit_registration_course = Curriculum.objects.all().filter(
                batch=2016, sem=6
            )
        teaching_credit_registration_course_data = serializers.CurriculumSerializer(
            teaching_credit_registration_course, many=True
        ).data
        ...
        for i in course_list:
            ...
            for j in list(instructors):
                ...
                presents = obj.student_attendance_set.all().filter(
                    instructor_id=j, present=True
                )
                absents = obj.student_attendance_set.all().filter(
                    instructor_id=j, present=False
                )
                pr += len(presents)
                ab += len(absents)
...
def add_thesis(request):
    ...
    if profile.user_type == "student":
        if not "thesis_topic" in request.data:
            return Response(
                {"error": "Thesis topic is required"},
        ...
        if "supervisor_id" in request.data:
            try:
                supervisor_faculty = User.objects.get(
                    username=request.data["supervisor_id"]
                )
                supervisor_faculty = supervisor_faculty.extrainfo
                request.data["supervisor_id"] = supervisor_faculty
            except:
                return Response(
                    {"error": "Wrong supervisor id. User does not exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
        ...
        if "co_supervisor_id" in request.data:
            try:
                co_supervisor_faculty = User.objects.get(
                    username=request.data["co_supervisor_id"]
                )
                co_supervisor_faculty = co_supervisor_faculty.extrainfo
                request.data["co_supervisor_id"] = co_supervisor_faculty
            except:
                return Response(
                    {"error": "Wrong co_supervisor id. User does not exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
        ...
        return Response(
            {"error": "Cannot add thesis"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def approve_thesis(request, id):
    ...
    if profile.user_type == "faculty":
        try:
            ...
            return Response(
                {"error": "This thesis does not exist"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if "member1" in request.data:
            try:
                ...
                return Response(
                    {"error": "Wrong username of member 1. User does not exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"error": "Member 1 is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        if "member2" in request.data:
            try:
                ...
                return Response(
                    {"error": "Wrong username of member 2. User does not exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            return Response(
                {"error": "Member 2 is required"}, status=status.HTTP_400_BAD_REQUEST
            )
        if "member3" in request.data:
            try:
                ...
                return Response(
                    {"error": "Wrong username of member 3. User does not exist."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        else:
            member3 = None
        if not "approval" in request.data:
            return Response(
                {"error": "Approval value is required."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        elif request.data["approval"] != "yes" and request.data["approval"] != "no":
            return Response(
                {
                    "error": "Wrong approval value provided. Approval value should be yes or no"
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        serializer = serializers.ThesisTopicProcessSerializer(
            thesis, data=request.data, partial=True
        )
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_200_OK)
        ...
        return Response(
            {"error": "Cannot approve thesis"}, status=status.HTTP_400_BAD_REQUEST
        )
