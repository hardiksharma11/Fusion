...
def feeds(request):
    ...
    if request.method == "POST":
        ...
        if request.POST.get("add_qus"):
            ...
            if len(role_check) > 0:
                access = QuestionAccessControl.objects.create(
                    question=question,
                    canVote=True,
                    canAnswer=True,
                    canComment=True,
    ...
    user_tags = (
        tags.objects.select_related()
        .values("my_tag")
        .distinct()
        .filter(Q(user__username=request.user.username))
    )
    u_tags = (
        tags.objects.select_related()
        .all()
    ...
    hid = (
        hidden.objects.select_related("user")
        .prefetch_related(
            "question__select_tag",
            "question__likes",
            "question__dislikes",
            "question__requests",
        )
...
def Request(request):
    ...
    if request.is_ajax():
        html = render_to_string(
            "feeds/question_request_count.html", context, request=request
        )
...
def LikeComment(request):
    ...
    if request.is_ajax():
        html = render_to_string(
            "feeds/like_section_comment.html", context, request=request
        )
        return JsonResponse({"form": html})
...
def delete_post(request, id):
    ...
    if request.method == "POST" and request.POST.get("delete"):
        ques = AskaQuestion.objects.prefetch_related(
            "select_tag", "likes", "dislikes", "requests"
        ).filter(pk=id)[0]
        if ques.file:
...
def update_post(request, id):
    ...
    if request.method == "POST" and request.POST.get("update"):
        question = AskaQuestion.objects.prefetch_related(
            "select_tag", "likes", "dislikes", "requests"
        ).get(pk=id)
        question.subject = request.POST.get("subject")
...
def ParticularQuestion(request, id):
    ...
    """
    result = AskaQuestion.objects.prefetch_related(
        "select_tag", "likes", "dislikes", "requests"
    ).get(id=id)
    a_tags = (
        tags.objects.select_related()
        ...
        if request.POST.get("answer_button"):
            ...
            if form_answer.is_valid():
                ...
                context = {
                    ...
                    "j": u_tags.filter(Q(my_tag__icontains="Jabalpur-city")),
                    "k": u_tags.filter(
                        Q(my_tag__icontains="IIITDMJ-Rules-and-Regulations")
                    ),
                    "l": u_tags.filter(Q(my_tag__icontains="Academics")),
                    "m": u_tags.filter(Q(my_tag__icontains="IIITDMJ")),
                    "n": u_tags.filter(
                        Q(my_tag__icontains="Life-Relationship-and-Self")
                    ),
                    "o": u_tags.filter(Q(my_tag__icontains="Technology-and-Education")),
                    "p": u_tags.filter(Q(my_tag__icontains="Programmes")),
                    "q": u_tags.filter(Q(my_tag__icontains="Others")),
...
def profile(request, string):
    ...
    ques = (
        AskaQuestion.objects.prefetch_related(
            "select_tag", "likes", "dislikes", "requests"
        )
        .all()
        .filter(user=usr)
...
def upvoteQuestion(request, id):
    ...
    if request.is_ajax() and isupvoted == 0:
        ...
        return JsonResponse(
            {
                "done": "1",
                "votes": question.total_likes() - question.total_dislikes(),
            }
        )
    else:
        return JsonResponse(
...
def upvoteAnswer(request, id):
    ...
    if request.is_ajax() and isupvoted == 0:
        ...
        return JsonResponse(
            {
                "done": "1",
                "votes": answer.total_likes() - answer.total_dislikes(),
            }
        )
    else:
        return JsonResponse(
...
def admin(request):
    ...
    if request.method == "POST" and request.POST.get("addrole"):
        ...
        try:
            ...
            if len(role_check) == 0:
                role_check_role = Roles.objects.select_related().filter(
                    role__iexact=role
                )
    ...
    if request.method == "POST" and request.POST.get("unassignrole"):
        if request.POST.get("unassignrole_value"):
            try:
                role_unassign = Roles.objects.select_related().get(
                    role=request.POST.get("unassignrole_value")
                )
    ...
    if request.method == "POST" and request.POST.get("reassignrole"):
        if request.POST.get("reassignrole_value"):
            try:
                role_unassign = Roles.objects.select_related().get(
                    role=request.POST.get("reassignrole_value")
                )
    ...
    if request.method == "POST" and request.POST.get("unassignrole_update"):
        try:
            ...
            user_check = User.objects.get(
                username=request.POST.get("unassignrole_update")
            )
...
def administrativeView(request, string):
    ...
    try:
        ...
        redirect("/feeds")
    result = (
        QuestionAccessControl.objects.select_related("posted_by__user")
        .prefetch_related(
            "question__select_tag",
            "question__likes",
            "question__dislikes",
    ...
    for q in result:
        ...
        if hid.all().filter(user=request.user, question=q.question).count() == 1:
            hidd = 1
        if (
            q.question.dislikes.all().filter(username=request.user.username).count()
            == 1
        ):
            isdisliked = 1
        access_check = q
