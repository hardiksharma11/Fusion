...
class Constants:
    ...
    SUBTAG_LIST = (
        ...
        (
            "Aakriti-The-Film-Making-and-Photography-Club",
            "AAKRITI-THE FILM MAKING AND PHOTOGRAPHY CLUB",
        ),
        ...
        (
            "SAMVAAD-THE-LITERATURE-AND-QUIZZING-SOCIETY",
            "SAMVAAD-THE LITERATURE AND QUIZZING SOCIETY",
        ),
...
class AskaQuestion(models.Model):
    ...
    dislikes = models.ManyToManyField(
        User, default=1, related_name="dislikes", blank=True
    )
    requests = models.ManyToManyField(
        User, default=1, related_name="requests", blank=True
    )
    ...
    def __str__(self):
        return "{} added ----> {} in {}".format(
            self.user, self.subject, self.select_tag
        )
...
class AnsweraQuestion(models.Model):
    ...
    answers = models.ManyToManyField(
        User, default=1, related_name="answers", blank=True
    )
    total_answers = models.IntegerField(default=0)
    likes = models.ManyToManyField(
        User, default=1, related_name="answer_likes", blank=True
    )
    dislikes = models.ManyToManyField(
        User, default=1, related_name="answer_dislikes", blank=True
    )
    is_liked = models.BooleanField(default=False)
...
class Comments(models.Model):
    ...
    likes_comment = models.ManyToManyField(
        User, default=1, related_name="likes_comment", blank=True
    )
...
class Reply(models.Model):
    ...
    replies = models.ManyToManyField(
        User, default=1, related_name="replies", blank=True
    )
    ...
    def __str__(self):
        return "{} - replied on  - {} at {}".format(
            self.user, self.comment, self.replied_at
        )
...
class tags(models.Model):
    ...
    def __str__(self):
        return "%s is interested in ----> %s - %s" % (
            self.user,
            self.my_tag,
            self.my_subtag.subtag,
        )
...
class Profile(models.Model):
    ...
    profile_picture = models.ImageField(
        null=True, blank=True, upload_to="feeds/profile_pictures"
    )
...
class QuestionAccessControl(models.Model):
    ...
    question = models.ForeignKey(
        AskaQuestion, related_name="question_list", default=1, on_delete=models.CASCADE
    )
